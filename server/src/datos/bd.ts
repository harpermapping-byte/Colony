...
export class AlmacenDatosSqlite implements IAlmacenDatos {
  ...
  async obtenerOCrearJugador(nombre: string, saldoInicial = SALDO_INICIAL_JUGADOR): Promise<Jugador> {
    const existente = this.bd
      .prepare("SELECT id, nombre, farycoins, vida, vida_max, anatomia, oficio_1, oficio_2, cambios_oficio FROM jugadores WHERE nombre = ?")
      .get(nombre);
    if (existente) {
      return {
    ...
    return { id: Number(r.lastInsertRowid), nombre, farycoins: saldoInicial, vida: 100, vidaMax: 100, anatomia: null, oficio1: "", oficio2: "", cambiosOficio: 0 };
  }
...
export class AlmacenDatosPostgres implements IAlmacenDatos {
  ...
  async obtenerOCrearJugador(nombre: string, saldoInicial = SALDO_INICIAL_JUGADOR): Promise<Jugador> {
    ...
    const r = await this.pool.query<{ id: number; nombre: string; farycoins: number; vida: number; vida_max: number; anatomia: string | null; oficio_1: string; oficio_2: string; cambios_oficio: number }>(
      `INSERT INTO jugadores (nombre, creado_en, farycoins) VALUES ($1, $2, $3)
       ON CONFLICT (nombre) DO UPDATE SET nombre = EXCLUDED.nombre
       RETURNING id, nombre, farycoins, vida, vida_max, anatomia, oficio_1, oficio_2, cambios_oficio`,
