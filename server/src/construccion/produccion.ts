...
/** Resuelve cuánto se ha producido desde `estado.ultimoCalculo` hasta `ahoraMs` — nunca negativo, nunca por encima de `capacidadMax`. */
export function resolverProduccion(estado: EstadoProduccion, datos: DatosProduccion, ahoraMs: number): EstadoProduccion {
  ...
  const producido = (horas / datos.intervaloHoras) * datos.cantidadPorIntervalo;
  return { ...estado, stock: Math.min(datos.capacidadMax, estado.stock + producido), ultimoCalculo: ahoraMs };
}
