...
## Injertos y cruces de cultivos — HECHO (2026-08-30), con un diseño MÁS SIMPLE que el que se había cerrado aquí

**⚠️ Nota de la implementación real**: el diseño que estaba cerrado en este backlog (6 atributos numéricos 0-1 por cultivo, cruce como media+variación aleatoria, combinación abierta entre cualquier par, catálogo `cultivos.json` de especies permanentes renombrables) NO es lo que se construyó. Se implementó en su lugar el patrón de **recetas fijas de crafteo** que ya usa cualquier otro oficio del juego (herrero/sastre/joyero — `items/catalogo/recetas.json`, oficio `"botanica"`, mesa `mesa_injertos`): dos semillas concretas como insumo → una semilla híbrida concreta como resultado, sin aleatoriedad ni atributos numéricos. Motivo: reutilizar el mecanismo de crafteo YA probado (mesa + nivel de oficio + insumos) en vez de construir un sistema de genética simulada nuevo — mucho menos trabajo, mismo resultado jugable ("puedes crear variedades nuevas mezclando dos plantas"), pero **cierra menos posibilidades que el diseño original** (nada de combinación abierta ni especies infinitas generadas en caliente). Ver `docs/GDD_Agricultura.md` §4. Si el streamer quiere el sistema abierto original más adelante, este backlog sigue siendo la referencia de ese diseño — no se ha borrado, solo señalado como no-construido.
