...
## Anatomía y medicina — HECHO (2026-08-30)

Pedido explícito: profundizar el sistema de vida 0-100, acotar tipo de daño por arma, vincularlo a un oficio de médico real. Ver `docs/GDD_Anatomia.md`.

6 zonas de golpe (cabeza/torso/brazoIzq/brazoDer/piernaIzq/piernaDer, los pivotes ya existentes del rig) con sangrado/fractura/infección/amputación por zona. `tipoDano` nuevo en las 10 armas del catálogo (cortante/contundente/perforante — mágico/fuego reservados para cuando existan esas armas). Probabilidades por golpe conectado, pedidas literalmente: 20% sangrado + 1% amputación (tiradas independientes) por golpe cortante, 10% fractura por golpe contundente. Autocuidado (vendar/entablillar, cualquiera sin oficio) detiene el efecto al instante pero deja la zona "cicatrizando" varias horas reales con malus leve y riesgo de infección sin ungüento; cirugía (oficio `curandero`, ya existía sin mecánica — `mesa_diagnostico`/`mesa_cirugia`/`choza_curandero`) cura todo al instante y es la única vía para salir de estado crítico (<10% vida). Prótesis de madera sobre zonas amputadas. Sin tick de servidor — todo perezoso, mismo integrador que el hambre/sed. 5 recetas de crafteo nuevas (sastre/herbolista/carpintero/curandero/herrero).
...
- **Tier 4 — recolección primaria: alimentan a los oficios de arriba, pero necesitan nodos/mecánica de recolección nueva en el bake que hoy no existe**: minero (vetas de mineral/piedra — alimenta a herrero y picapedrero), leñador, montar a caballo (y otras monturas). **Agricultura y pesca YA HECHAS** (2026-08-30, `docs/GDD_Agricultura.md`/`docs/archivo/GDD_Pesca.md` — este último archivado 2026-09-01, sin pendientes reales) — no dependían del bake exterior, se resolvieron con construcción de parcela + inventario + cálculo perezoso. **Caza (con desollado/encurtido) y cría de animales YA HECHAS** (2026-08-30, `docs/GDD_Caza.md`/`docs/GDD_Ganaderia.md`) — trampero (pieles para peletero) queda cubierto por el desollado de la caza, no como oficio de recolección aparte.
- **Tier 5 — sin building en catálogo todavía, o dependen de sistemas que no existen**: herrero de armas, herrero de armaduras (subdividir herrería general — necesita equipables reales), tallador de piedra/picapedrero, peletero, vidriero, arquero/ballestero (fletcher), guarnicionero (sillas/arreos — detrás de "montar a caballo"), destilador/cervecero, constructor, carnicero, mercader ambulante (`carromato_mercader` ya existe), navegante, guardia/mercenario (`cuartel_guardia`/`arena_combate` ya existen sin profesión encima), explorador/cartógrafo (`mapa_mesa` ya existe). **Ya no pertenecen aquí, resueltos (2026-08-30)**: curandero/médico (`docs/GDD_Anatomia.md`, receta real + `mesa_diagnostico`/`mesa_cirugia` enganchadas), herbolista (receta de ungüento), cocinero (`docs/GDD_Cocina.md`), capitán de barco/carpintero de ribera (`docs/GDD_Barcos.md`).
...
**HECHO ENTERO (2026-09-01)**: el reclutador/oficina de empleo llegó con `docs/GDD_NPCs_Contratables.md`, y ese mismo pedido fusionó "transporte" DENTRO de ese sistema como un oficio de trabajador más — `transporte:contratar` (firma libre, sin coste) quedó RETIRADO, sustituido por contratar un NPC trabajador de oficio `"transporte"` (mismo coste/salario que herrero/cocinero/etc.) y `trabajador:asignarRuta`. La pieza de "trabaja una mesa de verdad" también está hecha (crafteo automático real, no un flag) — ver ese documento para el diseño completo, ya no queda nada de este párrafo pendiente.
...
- ~~Heridas/enfermedad como estado~~ HECHO (2026-08-30) — ver sección "Anatomía y medicina" más abajo.
...
- ~~Zonas de golpe por parte del cuerpo~~ HECHO (2026-08-30) — el sorteo de zona por golpe ya existe (ver "Anatomía y medicina"), aunque todavía uniforme entre las 6, sin ponderar por tamaño real ni por si hay armadura puesta en esa zona concreta (el equipo suma a la defensa general, no por zona todavía) — eso sigue abierto.
...
Activa (caña + cebo, orilla, boya con ventana de reacción a la picada) y pasiva (trampa/cangrejera/batea de almejas, producción pasiva reusando el mismo mecanismo de colmena/aserradero). Ver `docs/archivo/GDD_Pesca.md` (archivado 2026-09-01, sin pendientes reales). Sin distinción de bioma de agua por casilla todavía (agua dulce vs. mar) — tabla de capturas genérica hasta que el runtime lea bioma de agua.
...
## Automatización de producción estilo Factorio — idea anotada, sin diseñar

Pedido del streamer 2026-08-31 al revisar `msakuta/FactorishJS`/`FactorishWasm` (juegos reales de cintas transportadoras + ensambladoras + cadenas de receta, evaluados en el chat — el segundo activo, Rust/WASM, MIT): confirmado que la IDEA vale la pena anotar, aunque no se copie código (lenguaje/arquitectura distintos, y de hecho ya tenemos el equivalente FUNCIONAL con otra piel — ver abajo).

- Hoy la producción "automática" real ya existe en dos piezas separadas: `server/src/construccion/produccion.ts` (refinamiento PASIVO tier N→N+1 dentro de una construcción, tipo colmena/aserradero) y `transporte:contratar` (`RoomExteriorBase.ts`) — un contrato que pone a un NPC transportista a caminar en BUCLE real (A* de verdad, duración por viaje, carga por viaje) entre construcción de origen y destino/cofre, sin que el jugador lo lleve a mano. Es, en la práctica, una cadena de producción automatizada punto a punto — con un NPC caminando el mapa en vez de una cinta animada.
- Lo que NO tenemos y sí tiene Factorish: una RED de varias etapas encadenadas automáticamente sin que el jugador contrate cada tramo por separado (A→B→C con una sola configuración, no 2 contratos de transporte sueltos), y una representación visual de "flujo" (la cinta se ve moverse) en vez de un NPC yendo y viniendo.
- Si algún día se quiere ir más lejos que "hilera de contratos de transporte manuales", el diseño real pendiente es: ¿un oficio nuevo tipo "ingeniero/mecánico" que desbloquea encadenar contratos de transporte en una sola configuración (una "línea" declarada de una vez, no tramo a tramo)? ¿O construcciones nuevas tipo "cinta" que ocupan casillas del mapa y mueven ítems visualmente entre construcciones adyacentes (esto sí sería nuevo de verdad — hoy nada mueve props por el mapa fuera de jugadores/NPCs/fauna)? Sin decidir — solo queda anotado que la idea es buena y encaja con "las listas crecen": sería un catálogo nuevo de piezas de línea de producción, no tocar el crafteo/refinamiento ya existente.

## Profundizar el combate (crítico/esquiva/estados/cooldowns/zonas de paz) — idea anotada, sin diseñar

Pedido del streamer 2026-08-31 tras revisar `tech-leads-club/nj-mmo` (mismo stack EXACTO que nosotros: Node.js + TypeScript + Colyseus + Three.js sin framework de UI, MIT, activo, 179 archivos de test) contra nuestro propio `server/src/combate/combate.ts` real: confirmado que ahí sí hay una brecha honesta, no una excusa para copiar código ajeno.

- **Hoy** (`combate.ts`, 79 líneas): `daño = max(1, round(ataque - defensa))`, sin más — nada de crítico, esquiva/precisión, cooldowns por habilidad, efectos de estado (buffs/debuffs con duración), ni concepto de zona de paz o flag de PvP. Combate PROPIO por turnos en arena (`CombateSchema`, `combate:mover`/`combate:accion`/`combate:pasarTurno`) — no en tiempo real como nj-mmo, pero la fórmula plana es igual de superficial sea por turnos o en vivo, el paradigma turn-based no es la razón del hueco.
- **Referencia real revisada** (`nj-mmo`, `libs/game-core`, modelado sobre fórmulas de Lineage 2): daño físico/mágico separado, tirada de crítico, precisión vs esquiva (DEX), tiempos de lanzamiento interrumpibles al recibir daño, cooldown por habilidad, buffs/debuffs con duración (`abnormalTime`), flag de PvP/karma que condiciona el daño jugador-contra-jugador, zonas de paz que bloquean el combate entero. Todo esto vive en funciones puras separadas de las Rooms de Colyseus — mismo patrón "lógica pura testeada aparte" que ya usamos nosotros en `combate.ts`/`cultivo.ts`/`cadaveres.ts`, así que encajar algo así no rompe la arquitectura, solo hay que decidir los NÚMEROS.
- **Diseño real pendiente, a decidir con el streamer antes de tocar código** (cambio grande, CLAUDE.md): ¿qué atributos alimentan crítico/esquiva (¿los ya existentes de personaje, o unos nuevos)? ¿cooldowns por arma/habilidad ya existen a medias (`cooldownMs` en `items/catalogo/items.json`, sin consumidor real todavía — revisar antes de inventar uno nuevo)? ¿qué efectos de estado concretos queremos primero (aturdir, sangrado, veneno...)? ¿zona de paz = simplemente parcelas propias/asentamientos con jarl, o un concepto nuevo de mapa? Nada de esto se construye sin pactar los valores — solo queda anotada la referencia y el hueco real encontrado.
