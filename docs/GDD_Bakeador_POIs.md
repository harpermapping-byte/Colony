...
3. **Caminos principales**: A* desde los bordes del mapa al focal con
   ...
   nace un PUENTE. Rasterizados con `ANCHO_CALLE=2` (`rasterizarSegmento`) —
   **todo lo demás que pinta calle (ronda, ramales, conector puerta→calle,
   senda de reparación) usa el MISMO ancho desde §15**: por debajo de ~1.8
   de grosor el rasterizado puede dejar tramos que solo conectan en
   DIAGONAL, invisibles para el movimiento real (4-direccional) del juego —
   ver §15 para el bug real que esto causaba y cómo se cerró.
...
5. **Calles menores DESPUÉS de los monumentales**: los obligatorios del
   ...
   interior, solo tiers grandes/castillo, `ANCHO_CALLE` desde §15 — antes
   1.4) y los ramales A* estratificados (`ANCHO_CALLE` desde §15 — antes
   1 casilla) rodeando lo construido; después se coloca el resto.
6. **Edificios por FRENTES de calle**: cada casilla de calle con su
   ...
   abre senda A* (con puente si toca, `ANCHO_CALLE` desde §15), con un
   fixer estructural de "costuras diagonales" corriendo ANTES como red de
   seguridad y un último recurso que descarta el edificio (nunca deja el
   bake roto) si de verdad no hay forma de conectarlo — ver §15.
...
## 15. Anchura y conectividad REAL de calles — el camino principal podía cruzar un río en un paso puramente diagonal, invisible para el movimiento 4-direccional del juego (2026-09-12, investigación previa por otro agente, implementado y verificado en esta pasada)

**Bug real, reproducido con el bakeador ANTES de tocar nada**: `aldea_pequena` con variante geográfica "rio" y semillas `"s1"`/`"s13"` (de una muestra de 60 semillas con río) generaban una ciudad con una puerta de edificio inalcanzable — `hornearCiudad` revienta con excepción (`validarCiudad` lo reporta, `index.js` hace `throw`). Barrido de 60 semillas antes del fix: **2/60 fallos** (ambos "rio", ~9% de las semillas con río en este tier). Causa exacta: `ANCHO_CALLE=2` (`rasterizarSegmento`) ya se usaba para los caminos PRINCIPALES desde el diseño original y es robusto incluso cruzando agua en diagonal pura (confirmado con una prueba dirigida: 0 casillas sin vecino 4-direccional tras rasterizar cientos de segmentos diagonales a ese grosor) — pero **la RONDA** (`carvarCallesMenores`, grosor fijo `1.4`), **los RAMALES** (pintados casilla a casilla, 1 de ancho), **el CONECTOR puerta→calle** de cada edificio (`colocarEdificio`, paso a paso con `Math.sign`) y **la senda de reparación de conectividad** por puerta NUNCA pasaban por `rasterizarSegmento` — con solo 1 casilla de ancho (o 1.4, que redondea a 1 o 2 filas según pura suerte), un tramo puede quedar conectado SOLO en diagonal: `floodDesde`/el movimiento real del servidor (`server/src/mundo/colisiones.ts::moverAABB`, mueve X e Y por separado) son estrictamente 4-direccionales, así que ese hueco diagonal es un muro real, no un atajo — mientras que el reparador por-puerta (que sí existía) usa un A* de **8 vecinos** que "ve" esa escalinata diagonal como camino válido, así que ni siquiera intentaba tallar nada: creía la puerta ya conectada.

**Arreglo, en el orden implementado**:
1. **Todo pintado a `ANCHO_CALLE`, reusando `rasterizarSegmento`** (nuevo helper `pintarPolilinea(puntos, grosor, ancho, alto, pintar)` en `geometria.js`, rasteriza cada par consecutivo de una polilínea con el mismo callback): la ronda pasa de grosor `1.4` a `ANCHO_CALLE`; los ramales, el conector puerta→calle y la senda de reparación pasan de pintar casilla a casilla a `pintarPolilinea` con la MISMA lógica de terreno que ya tenían (saltar muro/solar/adoquín/puente ya pintado; agua→puente; césped/tierra→camino o adoquín según dentro/fuera de la muralla) movida dentro del callback. Esto también elimina de raíz el "relleno de codo diagonal" manual que solo existía en la senda de reparación — `rasterizarSegmento` ya lo garantiza por construcción.
2. **`costeSenda` de lista NEGRA a lista BLANCA**: no conocía el id `"extramuros"` (introducido por el CONFINAMIENTO justo antes en el pipeline) y lo cruzaba gratis (coste 1) en vez de bloquearlo — una senda de reparación podía "encontrar ruta" saliendo del recinto por una zona que en el mapa final es sólida de verdad. Ahora cualquier terreno no reconocido explícitamente (`cesped`/`tierra`/`tierra_labrada`/`camino`/`adoquin`/`puente`, más agua a coste alto) es `Infinity` por defecto.
3. **Fixer estructural de "costuras diagonales"** (`repararCosturaDiagonal`, corre justo antes de la reparación por-puerta, 2 pasadas): recorre TODAS las casillas de calle; si una no tiene ningún vecino ORTOGONAL de calle pero sí uno DIAGONAL, es una costura rota — fuerza uno de los dos "codos" ortogonales a transitable (agua→puente, césped/tierra→camino), descartando cualquier codo que sea muro/solar_edificio real o esté ocupado por árbol/decoración colisionable (no hay nada seguro que convertir ahí sin corromper geometría real), y prefiriendo el codo que NO sea agua si hay elección.
4. **Reparación por-puerta reforzada, con dos mejoras reales sobre el plan original**: (a) si el A* de 8 vecinos no encuentra ruta con el límite de exploración por defecto (200000, que ya cubre de sobra cualquier tier actual — el mayor, capital_jarl a 400×400, son 160000 casillas), reintenta una vez con `ancho*alto` (explora el recinto completo); (b) **tras tallar (o si no hubo senda), se reverifica la conectividad REAL con `floodDesde` (4-direccional) antes de dar la reparación por buena** — no basta con que `aEstrella` (8 vecinos) devolviera una `senda` no nula, porque esa senda puede ser exactamente el mismo espejismo diagonal que el punto 3 no pudo cerrar del todo (visto de verdad en `capital_jarl`: una calle puede quedar encajonada entre dos solares reales con AMBOS codos bloqueados, un residual estructural aceptado — `validarCiudad` sigue limpio porque ninguna puerta depende de esa costura concreta). Si tras todo esto la puerta sigue sin ser alcanzable de verdad, el edificio se saca de `ciudad.edificios` y se añade a `ciudad.descartados` (mismo criterio ya aceptado por el streamer para "sin sitio": una ciudad válida con un edificio menos gana a un bake que revienta).
5. **Bug adicional encontrado ampliando la muestra de semillas de los tests (no parte del plan original, real y arreglado en la misma pasada)**: un árbol o pieza de decoración COLISIONABLE (`ciudad.arboles`/`ciudad.deco`, catálogo `colision:true`) plantado justo sobre el ÚNICO conector de un edificio bloquea de verdad (`validarCiudad` ya lo trataba así con sus "tapones" temporales antes de su propio flood), pero el bucle de reparación usaba un `floodDesde` CIEGO a la decoración — daba la puerta por "ya alcanzable" y nunca intentaba repararla, mientras `validarCiudad` la seguía reportando rota (reproducido con `capital`/semilla "barrido-5", `casa_gremio` — un edificio OBLIGATORIO). Cerrado dándole a `floodDesde` un 4º parámetro opcional `bloqueados` (duck-typed `.has(k)`) y pasándole `{has: k => esArbol.has(k) || decoSolida.has(k)}` en las dos llamadas del bucle de reparación — mismo criterio que ya usaban `validarCiudad` y `costeSenda`.

**Verificación (todo corrido de verdad, no afirmado)**:
- Regresión directa `aldea_pequena/rio` semillas `"s1"`/`"s13"`: `validarCiudad` pasa de reportar 2 y 4 errores respectivamente a `[]` en ambas.
- Barrido de 60 semillas (mismo que reprodujo el bug): **0/60 fallos** (antes 2/60).
- Barrido de 40 semillas × 8 tiers (320 generaciones, seeds `"sweep1".."sweep40"` Y por separado `"barrido-1".."barrido-40"`, las mismas que usa el test ampliado): **0/320 fallos** en ambos conjuntos (antes: 4/320, con `aldea_pequena` y `asentamiento_hostil` fallando — este último con una semilla que llegó a reportar 11 puertas inalcanzables a la vez).
- `ciudades/test/ciudad.test.js` ampliado de 13 a 15 tests: el barrido de validez pasa de 2 semillas por tier a un número diferenciado por coste (34 para los 5 tiers donde el bug se reprodujo de verdad — radio pequeño, pocas puertas, sin malla redundante — y 10 para los 3 tiers grandes que en un barrido de 40 semillas nunca fallaron y son mucho más caros de generar, ver comentario en el propio test para la cifra exacta y el porqué); nuevo test de regresión directa con `"s1"`/`"s13"`; nuevo test de "anchura" que recorre CADA casilla de calle del mapa buscando costuras diagonales EVITABLES (mismo criterio del fixer — si algún codo era convertible y no se convirtió, es un bug real; si los dos codos posibles están bloqueados por geometría real, es el residual estructural aceptado de `capital_jarl` documentado arriba). Los 15 tests pasan; tiempo total de la suite ~100-110s (antes ~33s con solo 2 semillas por tier en el test de validez).
- **Impacto en `ciudad.descartados` — real, medido, no ignorado** (mismo barrido de 40 semillas × tier, contando también los edificios "degradados" a una parcela reservada genérica por el mecanismo de red de seguridad ya existente, para no subestimar el efecto): `aldea_pequena` 9.6%→6.3% (MEJORA), `aldea` 0.8%→1.2%, `pueblo` 0.3%→1.0%, `capital` 5.5%→11.9%, `capital_jarl` 0.0%→0.2%, `castillo` 24.8%→29.4%, `asentamiento_hostil` 1.4%→0.6% (MEJORA), `gran_capital` 0.0%→0.0%. El aumento real está concentrado en `capital` y `castillo` (calles más anchas dejan menos hueco en tiers ya ajustados) — `castillo` ya partía de una tasa de descarte alta (24.8%) por diseño (radio pequeño con muchos obligatorios); ninguno de los 8 tiers deja de generar ciudades JUGABLES Y VÁLIDAS, y 2 de los 8 tiers mejoraron. **0 edificios** cayeron por la vía nueva "conectividad irreparable" (punto 4 del arreglo) en los 320 bakes del barrido — el único caso real encontrado (`casa_gremio`) fue el bug de decoración del punto 5, ya cerrado.
- Verificación VISUAL con `overview.png` real (`node ciudades/src/index.js <tier> <semilla> <carpeta>`), comparando el mismo tier+semilla antes/después con `git stash`: `aldea_pequena/s1` pasa de reventar con excepción a completar limpio; `pueblo/vcheck1` y `capital/vc1..vc5` muestran la ronda y los ramales visiblemente más anchos (2px reales en vez de 1px) sin romper el resto del layout, con el recuento de edificios/descartados exacto documentado en las capturas de esta sesión.
- `baker/src/instanciasPOI.js` (que llama a `hornearCiudad`/`generarCiudad` para asentamientos civiles Y campamentos hostiles) verificado con un bake real de `baker/config/ejemplo-rapido.json`: completa igual que antes (mismo único aviso preexistente de "POIs sin ruta", confirmado idéntico con y sin el fix — pertenece al trazador de caminos del baker EXTERIOR, no a `ciudades/`), sin cambios de interfaz (`hornearCiudad`/`generarCiudad`/`validarCiudad`/`cargarAsentamientos` mantienen su firma; `pintarPolilinea` es un export nuevo y aditivo de `geometria.js`, y el 4º parámetro de `floodDesde` es opcional).
- `interiores/test/catalogo.test.js` y `mazmorras/test/mazmorra.test.js` sin regresión (ninguno de los dos toca `ciudades/`, corridos como sanity check por compartir `geometria.js`/`azar.js`/`catalogo.js` transitivamente).
- **NO se tocó** `interiores/`, `server/` ni `client/` — el contrato de exportación de sectores (formato de chunk, índice, capa vectorial) no cambió en ningún campo.

Detalle completo del código (comentarios inline) en `ciudades/src/generar.js` (bloque "FIXER ESTRUCTURAL de costuras diagonales" y la reparación de conectividad por puerta) y `ciudades/src/geometria.js` (`pintarPolilinea`).

## 16. Decoración urbana de ciudades: los 21/32 objetos que faltaban ya tienen `.glb` real (2026-09-12)

Cierra el gap de arte menor que una auditoría de solo-lectura de una sesión anterior había dejado documentado en CLAUDE.md ("decoración urbana de ciudades 21/32 sin `.glb` — necesita EXTENDER el generador, no solo relanzarlo, `taller-vox/generar_modelos.js` nunca lee `ciudades/catalogo/decoracion.json`"): de los 32 ids reales de `ciudades/catalogo/decoracion.json` (bancos, farolas, puestos de mercado, carros, jaulas, vallas...), 11 ya tenían `.glb` aprobado en `assets/interiores/` (heredados del mobiliario de interiores/`generar_hitos_plaza.js` — pozo/fuente/estatua/barril/silla/mesa/sacos/cestas/abrevadero/maceta), quedaban 21 sin arte, cayendo a la caja de color placeholder de siempre.

**Mismo patrón exacto que `generar_hitos_plaza.js`/`generar_puerta_asentamiento.js`** (los dos precedentes reales para "pieza suelta de `ciudades/`, sin mueble de interiores/ que reusar"): `taller-vox/generar_decoracion_urbana.js` (nuevo, script AUTÓNOMO — `generar_modelos.js` NO es una librería importable, ejecuta su generación completa en cuanto se importa) con su propio `Builder`/`sombrear`/PRNG (`crearPRNG` de `interiores/src/azar.js`), leyendo DIRECTAMENTE `colorDebug`/`dimensiones` de `ciudades/catalogo/decoracion.json` (nunca proporciones inventadas) y 3 variantes por semilla para cada uno de los 21 ids.

Dos familias de geometría adaptadas de arquetipos ya probados en `generar_modelos.js` (banco SIN respaldo derivado de `generarAsiento`, tinaja con el mismo "abombado" de capas que `barril`/`tinaja` de `generarContenedorBajo` pero con bandas de arcilla en vez de aros de metal) y el resto son arquetipos NUEVOS por familia: "poste+travesaño" (valla/amarradero), "mostrador+toldo" (puesto_mercado/tenderete_comida), "caja de listones" (jaula_vacia/gallinero_jaula), y la familia más laboriosa del lote — "carro con ruedas" (carreta/carro_mano/carromato), la primera vez que este repo modela un vehículo con ruedas.

**3 bugs reales encontrados verificando las capturas, no dados por buenos con solo la validación estructural** (mismo criterio de esta sesión: la validación de `.glb` confirma que el archivo es válido, nunca que se VEA bien):
1. **`mostradorConToldo` (puesto_mercado/tenderete_comida): el toldo colapsaba a una loncha de 1-2 vóxeles** — `postH=gy*0.9` dejaba solo un 10% de la altura total para el techo de tela, así que el puesto se veía como dos postes con una barra fina encima, sin ningún plano inclinado reconocible. Corregido a `postH=gy*0.58`.
2. **El mismo `mostradorConToldo`, segundo bug tras arreglar el primero: el toldo dejaba los postes DELANTEROS sin tocar** — la capa MÁS BAJA del techo solo cubría la franja trasera (`zTop` variaba con la altura, encogiéndose hacia la parte de atrás en vez de cubrir toda la profundidad desde el principio), así que el poste delantero se veía flotando, desconectado de la caseta. Corregido a que cada capa cubra SIEMPRE la profundidad entera (0..gz-1), solo desplazándose un poco hacia dentro en X según sube — un tejadillo escalonado real, apoyado en los 4 postes desde la primera capa.
3. **`ruedaPlana` (carreta/carro_mano/carromato): las ruedas salían como cubos casi sin redondear** — la primera versión reusaba el "abombado" senoidal del truco de barril (pensado para el PERFIL LATERAL de un tonel, con un inset de solo `radio*0.3` en los extremos) pero una rueda se ve casi DE FRENTE a la cámara (su cara circular, no su perfil), así que ese inset mínimo se leía como un cubo con las esquinas apenas biseladas — cero apariencia de rueda. Corregido con un rasterizado REAL de círculo (fila a fila, semiancho=`sqrt(radio²-dy²)`) + aro de 1 vóxel más oscuro en el borde + radios en cruz — confirmado visualmente como una rueda de carro reconocible, con cubo y radios. De paso, las ruedas ganaron su propio margen de profundidad (`margenZ`, protruyendo hacia fuera de las paredes laterales) — sin él, la rueda vivía en el MISMO plano Z que la pared y quedaba enterrada dentro de la caja de carga, sin sobresalir nada (mismo `carro_mano` con una única rueda ancha pegada al borde delantero, con su propio margen `radioRuedaMano` en X para que también sobresalga por delante).

**Enganche rápido** (mismo criterio ya usado repetidas veces en este proyecto para lotes grandes de arte procedural — hitos de plaza, puerta de asentamiento, herramientas/armas, objetos de expositor, edificios de ciudad): los 63 `.glb` (21 ids × 3 variantes) se subieron DIRECTOS a `assets/interiores/` sin revisión pieza a pieza — el streamer puede pedir rehacer cualquiera de las 21 tras verla en persona. Categoría `"interiores"` (no `"edificios"`) porque `ciudades/` exporta la decoración urbana con `t:"m"` (mismo tipo que mobiliario), `sectorVisual.ts` ya prueba genéricamente `assets/interiores/<id>_NN.glb` para ese tipo — cero cambio de cliente/servidor.

**Verificado**: los 63 `.glb` validados estructuralmente con `taller-vox/validar_glb.js` (magic/JSON/BIN/accessors correctos, sin mesh vacío, sin NaN en las cajas — 0 inválidos en las 6 rondas de regeneración de esta pasada) + verificación VISUAL real de las 21 piezas (Playwright + `client/test/verGlbAislado.html`, script de captura aislado desechable siguiendo el mismo patrón que `colisionEdificioAisladoCaptura.mjs`/`nieveAisladoCaptura.mjs` — no comiteado, fuera del alcance de esta pasada) confirmando que ninguna es una caja de color: banco de madera/piedra con patas y tablón reales, valla con postes y travesaños, jaula/gallinero con barrotes espaciados, farola con mástil y cabeza de farol, puesto de mercado y tenderete de comida con mostrador+toldo escalonado real, carreta/carromato con ruedas redondas reconocibles (cubo+radios) y varal, carromato con lona curva sobre la caja, tinaja con forma de tonel/vasija, antorcha de poste con trípode+llama, cartel de tienda colgado con brazo de hierro, cartel de poste con tablillas, escalera de mano inclinada con peldaños, tendedero con paños colgando, charco liso sin ningún patrón repetido, cubo de madera con asa en arco, leña apilada en dos tonos. `taller-vox/generar_decoracion_urbana.js` no toca ningún test existente (pieza nueva y autocontenida, sin consumidores en `server/`/`client/` — el catálogo de decoración urbana no tenía test dedicado antes de esta pasada, ninguno se rompe). **Sin verificación en el mapa principal en vivo** (haría falta un rebake completo de Vetrheim, o al menos de una ciudad de prueba, para ver estas piezas puestas de verdad en una calle — mismo criterio honesto del resto de esta sesión) — pendiente de que el streamer lo confirme jugando tras el próximo rebake.
