...
**Fase 3b — arado/cultivadora montados (§9.3): IMPLEMENTADA Y VERIFICADA (2026-09-04).** Automatizan `cultivoCasilla:labrar`/`plantar` disparados por el movimiento del conjunto en vez de un click manual, sobre la base de Fase 3a sin tocarla:

- **Catálogo** (`items/catalogo/items.json`, `tipo:"carro"`, `categoria:"labranza"`): `arado_madera` (tier 2, sin carga propia) y `cultivadora_semillas` (tier 3, con un semillero pequeño 3×2 que REUSA el mismo mecanismo de `carro:meterCarga`/`sacarCarga` de un carro de materiales — `crearContenidoInicial` trata "labranza" igual que "materiales" cuando el catálogo declara `capacidadContenedor`, cero código nuevo ahí). Recetas de ingeniero en el mismo commit.
- **`server/src/rooms/base/RoomExteriorBase.ts`**: `apero:comenzarLabrar`/`comenzarCultivar` (solo el CONDUCTOR puede activar su propio apero, exige `categoria:"labranza"`) y `apero:detener`. Nueva rama de velocidad: mientras `aperoEnUsoPorConjunto` tiene el conjunto, la velocidad de conjunto normal se multiplica ADICIONALMENTE por `FACTOR_APERO_EN_USO=0.4` (§6, "cuesta más moverse", sin el suelo de seguridad `>=VEL_ANDAR` del carro normal — aquí sí puede ir más lento que a pie, es el precio de la automatización). El efecto se dispara desde la MISMA pasada de sincronización de conjuntos que ya existía (tras fijar `esquema.x/y` al conductor), comparando la casilla actual contra la última procesada (`ultimaCasillaAperoPorConjunto`) para actuar solo al ENTRAR en una casilla nueva — border especial: al activar el apero se borra esa marca, así que la casilla de partida se labra/cultiva DE INMEDIATO, sin esperar a moverse. `aplicarAperoLabrarAutomatico` reusa exactamente las mismas reglas que `cultivoCasilla:labrar` (parcela propia del conductor, suelo libre, sin colisión); `aplicarAperoCultivarAutomatico` barre un radio de `RADIO_CULTIVADORA=2` casillas sembrando cada casilla YA labrada del conductor con la semilla cargada en el semillero, hasta agotarla. Ambos mandan el MISMO evento (`cultivoCasilla:labrada`/`plantada`) que ya recibía un click manual — el jugador ve feedback real, no es una acción muda. Al desmontar el conductor (`desmontarConjuntoSesionId`) el apero en uso se para solo (sin conductor no tiene sentido seguir "activo").
- **`server/test/apero.e2e.mjs`** (nuevo): servidor Colyseus real sobre `testflat`. Cubre el flujo completo: arado enganchado+montado labra la casilla de partida sin moverse y cada casilla nueva mientras avanza (8 casillas distintas en la prueba real); `apero:detener`; desenganchar; cultivadora enganchada con 5 semillas cargadas siembra automáticamente las casillas ya labradas en su radio (5/5 en la prueba real, consumiendo las 5 semillas); y una comprobación de que lo sembrado por la cultivadora es de verdad persistente (reintentar plantar ahí se rechaza `ya_sembrada`, no un evento cosmético). Verificado junto al resto: **suite completa de servidor 1122/1122**, `tsc --noEmit` limpio, y los e2e de todas las fases previas (`barcos`/`monturas`/`carros`/`carrosCarga`/`cultivoCasilla`/`testZoneDebug`) siguen en verde.

**Fase 4 — catálogo completo (§8.1/§11): IMPLEMENTADA Y VERIFICADA (2026-09-04).** Los últimos 2 ítems nombrados en la tabla de §11 que faltaban — todos los demás (nivel 2 y 3 completos) ya se habían implementado en Fases 1-3:

- **`diligencia_4`** (tier 3, 4 asientos, capoteada) y **`carruaje_noble_5`** (tier 4, 5 asientos, capoteado y tapizado, "el más caro del catálogo" — pedido literal). Recetas de ingeniero (`tela_hilada` por la capota, `carruaje_noble_5` suma `cuero_curtido` por el tapizado) en el mismo commit. `capotado:true` sigue siendo un campo reservado sin efecto mecánico (mismo criterio "placeholder" del resto del catálogo — GDD §7).
- **Hallazgo real**: estos son los DOS PRIMEROS carros con más de 2 asientos del catálogo — el mecanismo de varios pasajeros (`conductorSessionId` + `ocupantesDeConjunto`, construido en Fase 1) nunca se había probado con jugadores de verdad montados a la vez, solo con el conductor solo. **`server/test/carroPasajeros.e2e.mjs`** (nuevo) cierra ese hueco: un cochero engancha+monta una `diligencia_4`, TRES pasajeros más se montan (`conjunto:montar` auto-apuntado) hasta llenar las 4 plazas, un 5º jugador queda fuera (sin hueco), y al mover el cochero los 3 pasajeros se desplazan de verdad junto al conjunto (offset fijo, sin mandar su propio input) — probado con 5 clientes `colyseus.js` reales conectados a la vez. Verificado junto al resto: **suite completa de servidor 1122/1122**, `tsc --noEmit` limpio, y los e2e de todas las fases previas siguen en verde.

**Fase 5 — trabajador de transporte (§12): IMPLEMENTADA Y VERIFICADA (2026-09-04).** Asignación de montura/conjunto propio a un trabajador de oficio "transporte" (`docs/GDD_NPCs_Contratables.md`), sobre la base de Fases 1-4 sin tocarlas:

- **`server/src/datos/bd.ts`**: `NpcTrabajador` gana `mascotaAsignadaId`/`conjuntoAsignadoId` (nullable, mutuamente excluyentes — "qué usa para desplazarse"), columnas `mascota_asignada_id`/`conjunto_asignado_id` en `npcs_trabajadores` (migración `ALTER TABLE` en ambos backends, mismo patrón que el resto del sistema); `IAlmacenDatos` gana `asignarMonturaNpcTrabajador(id, mascotaId, conjuntoId)` y `actualizarDuracionContrato(id, duracionViajeSeg)` (SQLite y Postgres).
- **`server/src/mundo/agentes.ts`**: `agregarAgenteTransportista` gana un `velocidadMultiplicador?` opcional, reusando el mismo campo `NpcBakeado.velocidad` que ya existía para "el corredor especial" — `undefined`/`1` sigue siendo "a pie", cero cambio de comportamiento para el resto de agentes.
- **`server/src/rooms/base/RoomExteriorBase.ts`**: dos helpers nuevos —
  - `velocidadEfectivaTrabajador(fila)`: `VEL_NPC` (a pie) por defecto; con `mascotaAsignadaId`, la `velocidadMontura` de esa especie (suelo `VEL_NPC`); con `conjuntoAsignadoId`, esa velocidad × `FACTOR_CARRO_NORMAL` (mismo factor que aplica al jugador conduciendo, §6) — a propósito SIN el suelo `VEL_ANDAR+0.25` del jugador humano, ya que un NPC nunca anduvo a esa velocidad.
  - `capacidadEfectivaTrabajador(fila, itemId, cargaPorDefecto)`: `CARGA_POR_VIAJE_TRANSPORTE` por defecto; con un `conjuntoAsignadoId` cuyo carro es categoría "materiales", sustituye por la capacidad REAL de su rejilla vacía (`capacidadLibre` sobre un `crearContenedor` del tamaño del catálogo) — una carreta carga mucho más que a lomos. Se recalcula en cada resolución del contrato (`resolverUnContrato`), nunca se cachea, así que un cambio de conjunto se refleja de inmediato en el siguiente viaje económico.
  - `crearRutaTransporte` (usada tanto al firmar una ruta nueva como al recalcular) usa ambos helpers en vez de las constantes fijas de antes.
  - **`trabajador:asignarMontura {trabajadorId, mascotaId?, conjuntoId?}`** (nuevo mensaje): exige propiedad del trabajador (`trabajadorPerteneceA`) y de la mascota/conjunto (`listarMascotas`/`listarConjuntosTiroDe` filtrados por dueño), rechaza mandar `mascotaId` Y `conjuntoId` a la vez (mutuamente excluyentes, error explícito) y rechaza una mascota/conjunto que el propio jugador esté usando en ese momento (`montadoPorSesion`/`conjuntosPorSesion`). Si el trabajador ya tenía una ruta activa, recalcula `duracionViajeSeg` (persistido vía `actualizarDuracionContrato`) y **respawnea el agente caminante con la nueva velocidad** — el CAMINO en sí nunca se recalcula (regla dura de `agentes.ts`, "nada de A* en vivo"), solo el ritmo al que se camina esa misma polilínea ya fija. Mandar ambos campos vacíos desasigna (degradación consistente a `VEL_NPC`/`CARGA_POR_VIAJE_TRANSPORTE`, sin fallo duro — mismo criterio del resto del proyecto).
- **`server/test/npcs_trabajadores_montura.e2e.mjs`** (nuevo): servidor Colyseus real + BD SQLite real + cliente `colyseus.js` real sobre el mapa PRINCIPAL. Cubre: comportamiento base sin montura asignada (duración/carga sin cambios); rechazo de `mascotaId`+`conjuntoId` simultáneos; rechazo de mascota ajena; asignar una montura EN CALIENTE (ruta ya activa) recalcula la duración Y el agente camina más rápido DE VERDAD — medido por **tiempo real de trayecto** (umbral de posición absoluto al salir del origen y al llegar al destino, no "velocidad instantánea por muestra": con un tramo corto y una pausa fija de 7s entre paradas del `GestorAgentes`, medir distancia-en-ventana-fija es engañoso, cualquier velocidad "pasa casi todo el tiempo parada"); asignar un conjunto reemplaza la mascota (mutuamente excluyentes, no acumulativo) y sigue siendo más rápido que a pie; desasignar degrada de vuelta a la velocidad base sin error; y un conjunto de materiales asignado ANTES de `asignarRuta` sustituye `cargaPorViaje` por la capacidad real de su rejilla (1200 frente al 10 plano, sobre `carro_materiales_pequeno`+miel). Verificado junto al resto: **suite completa de servidor 1122/1122**, `tsc --noEmit` limpio, y los e2e de todas las fases previas (`barcos`/`monturas`/`carros`/`carrosCarga`/`cultivoCasilla`/`apero`/`carroPasajeros`/`npcs_trabajadores_transporte`/`npcs_trabajadores_crafteo`) siguen en verde.
- **Nota de test unrelacionada, detectada de paso**: `server/test/npcs_trabajadores.e2e.mjs` tiene una aserción desactualizada (espera 10 oficios contratables, el catálogo real ya tiene 11 desde la fusión del oficio "transporte" en una fase anterior) — no es un efecto de esta fase, queda anotada como tarea aparte en vez de tocarse aquí.

Con esto se cierran las 5 fases de `docs/GDD_Carros.md` — el sistema de carros/arneses/aperos/agricultura de casilla/integración con trabajadores queda completo según lo pedido en §0-§13.
