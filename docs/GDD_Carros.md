...
2. **Fase 2 — categorías de carga**: materiales, muebles (con el `ContenedorMuebles` nuevo), animales, líquidos (con `verterLiquido`).
3. **Fase 3 — agricultura de casilla + aperos**: azada + `cultivoCasilla:labrar/plantar/cosechar` (§9.1-9.2) primero, verificado a mano ANTES de automatizarlo — luego arado/cultivadora (§9.3), y las ampliaciones de §10 si se aprueban.
