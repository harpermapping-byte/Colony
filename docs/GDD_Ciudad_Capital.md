...
- **Colocar los proyectos especiales del jarl en las parcelas grandes reservadas** depende de DOS cosas que no existen todavía: (a) que el sistema de construcción (`docs/GDD_Construccion.md`) se extienda más allá del Hub a regiones de `ciudades/` — bloqueante ya documentado en `GDD_Faccion_Bandidos.md` §8.1, sigue sin resolver; (b) que cada edificio especial del backlog (`docs/Backlog_Mecanicas_Futuras.md` § "Proyectos especiales del jarl") se defina de verdad como `construible` en `interiores/catalogo/tipos_edificio.json` — hoy son solo una lista de nombres con propuesta de mecánica, no entradas de catálogo. Este documento **no** los define como construibles ni genera sus interiores — fuera de alcance a propósito.
...
- **Construcción/parcelas fuera del Hub** y **combate** (las otras dos dependencias de `GDD_Faccion_Bandidos.md` §8.1) siguen exactamente igual de sin resolver que antes — este documento **no** las destraba, solo el generador de capital en sí.
- El orden de dependencias de `GDD_Faccion_Bandidos.md` §8.3 pasa a: ~~1. Generador de capital~~ (hecho, este documento) → 2. Construcción/parcelas en regiones → 3. Crafteo por planos → 4. Combate.
