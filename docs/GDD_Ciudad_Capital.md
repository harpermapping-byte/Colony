...
## 5ter. Proyectos especiales "donde quiera" — colocador del jarl (pedido 2026-08-31)

*"esto es para que cuando construya también en el interior de la ciudad capital los edificios especiales los coloque donde quiera"* — hasta ahora un proyecto especial SOLO podía levantarse sobre una parcela `tipo:"especial"` reservada de antemano (§3, vía el "construir" normal — sigue funcionando igual, no se retira). Este pedido añade un segundo camino, sin marcar nada a mano primero: los 14 proyectos especiales se incorporaron a `cargarCatalogoPlantillas()` (antes solo `aserradero`-estilo) y `validarColocacionPlantilla` los deja pisar cualquier parcela existente (a diferencia de una plantilla de producción normal, que sigue exigiendo estar fuera) — mismo tope de UNO-por-asentamiento que ya tenía la vía de construir normal, compartido entre las dos vías. Detalle técnico completo, incluida la generalización del validador y el colocador cliente nuevo (`colocadorPlantillas.ts`, tecla Z): `docs/GDD_Produccion.md` §3bis.
