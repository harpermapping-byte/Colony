...
- **Colocar los proyectos especiales del jarl en las parcelas grandes reservadas** depende ahora SOLO de UNA cosa: que cada edificio especial del backlog (`docs/Backlog_Mecanicas_Futuras.md` § "Proyectos especiales del jarl") se defina de verdad como `construible` en `interiores/catalogo/tipos_edificio.json` — hoy son solo una lista de nombres con propuesta de mecánica, no entradas de catálogo. Tampoco hay todavía ningún tratamiento distinto para las parcelas `tipo:"especial"` frente a las `"normal"` (el dato viaja en el id de la parcela pero nada lo consume aún). Este documento **no** los define como construibles ni genera sus interiores — fuera de alcance a propósito.
