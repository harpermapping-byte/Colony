...
1. Instala lo que falte: **Git** (`Git.Git`), **Node.js** (`OpenJS.NodeJS.LTS`), **cloudflared** (`Cloudflare.cloudflared` forzando `--installer-type wix`: el MSI se registra en el PATH del sistema y permite `cloudflared service install`, el portable no). Deliberadamente **sin** `--scope machine` en Git: en su manifiesto real las filas de scope *user* y *machine* son idénticas (mismo instalador, mismos switches), así que ese flag no cambia dónde acaba Git y solo puede hacer que winget no encuentre fila y falle — lo que de verdad lo manda a `Program Files` y al PATH del sistema es la elevación, que `instalar.bat` ya garantiza.
...
**Trampa de Windows PowerShell 5.1 que hay que respetar al tocar estos scripts** (rompió el instalador en la primera ejecución real): con `$ErrorActionPreference = "Stop"`, PowerShell 5.1 convierte **cualquier** línea que un programa externo escriba por la salida de error en un error **fatal** — y `npm`, `git`, `pm2` y `psql` escriben ahí hasta sus avisos inofensivos (`npm warn install-scripts …` bastó para matar el instalador). Por eso todas las llamadas a programas externos pasan por el envoltorio `EjecutarNativo`, que baja la preferencia a `Continue`, fusiona la salida de error con la normal y decide **por el código de salida**, no por si el programa dijo algo. Ese comportamiento es exclusivo de PowerShell 5.1: en PowerShell 7 (con el que se prueba en desarrollo) no se reproduce.

**Aviso pendiente para el futuro, no bloqueante hoy**: npm avisa de que hay paquetes con scripts de instalación "no cubiertos por allowScripts" — en npm 11 es **solo informativo** y los scripts sí se ejecutan, pero **npm 12 los bloqueará por defecto**. En este repo son `esbuild` (postinstall, y lo necesita `vite build`: sin él el cliente no se compila) y `msgpackr-extract` (install, acelerador nativo opcional de Colyseus, con respaldo en JS). Cuando el PC pase a npm 12 habrá que aprobarlos (`npm approve-scripts`) o el build del cliente empezará a fallar.
...
**Bug real encontrado y cerrado (2026-09-10): el dominio raíz daba 522 "Connection timed out" pese a tener la ruta del túnel bien configurada** — la causa NO era el túnel ni el servidor (el servidor respondía perfecto por `curl http://localhost:2567/estado`, el túnel estaba "Healthy"): en **DNS → Records** del dominio, `colony-streamer.online` (raíz) seguía teniendo un registro **`A` apuntando a `162.255.119.140`** — la IP del parking page de Namecheap, un registro que se queda puesto por defecto al comprar el dominio y que nadie borró al montar el túnel (`www.colony-streamer.online` tenía el mismo problema, un `CNAME` a `parkingpage.namecheap.com`). Cloudflare no puede tener a la vez un `A` y un `CNAME` para el mismo nombre, así que ese `A` viejo bloqueaba que el dominio apuntara de verdad al túnel — toda visita a la raíz se mandaba al parking de Namecheap (que no completaba la conexión como Cloudflare esperaba) en vez de al túnel, de ahí el timeout. Diagnosticado por descarte real (confirmado uno a uno: servidor vivo, túnel sano, Service `http://localhost:2567` y luego `http://127.0.0.1:2567` correctos, Live Logs del túnel sin UNA sola fila pese a docenas de peticiones reales — la pista de que el tráfico ni siquiera llegaba al conector) antes de mirar el DNS. **Arreglo**: borrar ese `A` (y el `CNAME` de `www.` si se quiere que funcione también) y crear un `CNAME` para el nombre raíz apuntando a `<tunnel-id>.cfargotunnel.com` (Proxied) — Cloudflare SÍ soporta CNAME en el apex de una zona que gestiona él mismo (CNAME flattening nativo), a diferencia de la mayoría de proveedores DNS. **Moraleja para la próxima vez que se monte un dominio nuevo desde cero en Cloudflare**: revisar SIEMPRE los registros DNS que trae el dominio recién comprado (parking del registrador) antes de dar por sentado que "publicar la ruta" en el túnel ya es suficiente — el túnel puede estar perfecto y el dominio seguir sin llegarle ni una petición.
...
**Verificado en el PC real del streamer (2026-09-10)**: el dominio raíz `colony-streamer.online` sirve el juego de verdad a través del túnel — tras cerrar el bug real del registro `A` de parking (ver arriba), el streamer confirmó carga correcta en su navegador.

Sin verificar todavía: el rendimiento real sirviendo assets a varios jugadores a la vez por la subida de casa; la Tarea Programada de Windows y el reinicio de las 8 h en su máquina.
