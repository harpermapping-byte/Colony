...
Pedido explícito: "el huevo tiene prop sprite y se puede ver en el suelo o en el nido" — sustituye el acumulador abstracto de "huevos" que tenía v1 (§6, `animal:recolectarProducto`) por objetos reales (`ObjetoMundoSchema`, mismo mecanismo que soltar/cadáveres/pesca) en el sitio del ave. Requiere un macho ADULTO de su misma pareja en la propiedad — sin él, no pone. Una vez por día de mundo por hembra elegible (gatea igual que leche/lana: `tieneComidaYAguaHoy`). Cantidad: **1 en el suelo si la propiedad no tiene un `nido` construido (literal — `gallinero` NO cuenta para esto pese a ser refugio válido para adquirir el animal en §4, comprobado en código: `resolverReproduccionAnimalesPropiedad` filtra `viva.objeto === "nido"` exacto, sin incluir `gallinero`), 1 a 3 si lo tiene** — pedido literal. El "nido" reusa la construible YA existente de v1, sin inventar un concepto nuevo. **El huevo siempre aparece junto al AVE que lo puso, nunca dentro del nido en sí** — el nido solo decide la cantidad, no la posición (si se quisiera que el huevo apareciera físicamente sobre el nido sería un cambio de posición aparte, no pedido).

**Corrección 2026-09-01**: "se puede ver en el suelo" no era del todo cierto hasta ahora — el huevo SÍ se creaba en `state.objetosMundo` (esta sección, sin cambios), pero ningún cliente pintaba ningún `objetosMundo` en absoluto (ni huevos ni ningún otro objeto soltado del juego) — hueco cerrado en §12.

### 11.4 Decisiones a confirmar con el streamer

- ~~"huevos" desaparece de `PRODUCTOS_GRANJA` (§6) — antes daba un acumulador invisible por `animal:recolectarProducto`, ahora son objetos físicos recogibles por cualquiera. A confirmar si eso es lo que se quería o si se prefiere mantener ambos caminos.~~ **Confirmado (2026-09-01), ver §12**: es el comportamiento querido, un solo camino (objeto físico, nunca acumulador). El streamer detalló además la recogida: clic sobre el huevo o sobre el nido, mismo menú de interacción genérico que el resto del proyecto.
...
- **Sin wiring de cliente (§10) para la CRÍA de animales** — sigue vigente: un `AnimalGranjaSchema` nuevo (cría nacida, adulto madurado) no se renderiza (§5 de la investigación de esta pasada: `AnimalGranjaSchema`/`state.animalesGranja` no tiene `onAdd` en ningún `game.ts`, ni las gallinas/ocas se mueven — son entidades estáticas, fuera de `server/src/mundo/agentes.ts`), el jugador lo descubre solo si vuelve a mirar la lista. **Los HUEVOS ya NO tienen este hueco, ver §12** — es un objeto de mundo genérico (`objetosMundo`), no un `AnimalGranjaSchema`, así que sí se renderiza desde esta pasada.
...
## 12. Recolección de huevos — nido y suelo, menú de interacción por clic (2026-09-01)

Pedido literal del streamer, tras confirmar §11.4: *"la idea es que sean un objeto como recolectable tanto del suelo como de nidos... las gallinas o aves dejan huevos en nidos o suelo, en el mueble nido dejarían MÁS huevos que en el suelo, click sobre el nido o sobre el huevo, recoger huevo, saldrá pantalla como tocar etc"*. La mecánica de CANTIDAD (nido = 1-3, suelo = 1) ya estaba implementada desde §11.3 — lo que faltaba entero era la mitad de cliente: ni un solo `objetosMundo` se pintaba, y "nido" (mueble ya colocable) no ofrecía ninguna opción al clicarlo.

- **`client/src/mundo/renderObjetosMundo.ts`** (nuevo) — render GENÉRICO de `state.objetosMundo`, no específico de huevos: es el mismo `ObjetoMundoSchema` que ya usan soltar manual, overflow de pociones/crafteo, y loot de cadáver que no cabe — ninguno de esos tenía malla tampoco, así que esta pasada cierra el hueco para los cuatro de una vez, no solo para huevos. Mismo patrón EXACTO que `construccion/renderConstrucciones.ts` (Map local, `añadirEstatico`/`quitarEstatico`, `mallas()`/`datosDeMalla()` para el raycast), pero sin huella/rotación — caja pequeña (0.35, más chica que un mueble a propósito) con el `colorDebug` real del item de `items/catalogo/items.json` (huevo: `#e8d8a8`). Alcance limitado al Hub por ahora, mismo límite ya aceptado para mesas de minijuego/asiento genérico (`docs/GDD_Personaje.md` §3.6ter) — Región/Interior quedan pendientes de que esas rooms tengan su propio seguimiento de estado en cliente.
- **Clic sobre un huevo suelto** — `game.ts` combina las mallas de `renderObjetosMundo` con las de `renderConstrucciones` en el MISMO raycast de clic (`menuInteraccion.ts`, nacido para instrumentos, `docs/GDD_Instrumentos.md`), resuelve primero si el impacto es un objeto de mundo y, si lo es, ofrece una única opción `"Recoger <nombre>"` (nombre real del catálogo, ya no solo huevo — cualquier objeto suelto futuro queda cubierto gratis) que manda `"coger"` — el mismo mensaje genérico SIN id que ya existía (auto-apunta al objeto suelto más cercano dentro de `RADIO_INTERACCION`, `RoomExteriorBase.ts::buscarObjetoSoltadoCercano`); no hace falta un mensaje "recoger ESTE huevo concreto" porque haber podido clicar la malla ya implica estar cerca de ella.
- **Clic sobre el mueble `nido`** — rama nueva en el mismo `if (datos.objeto === "...")` que ya usan telar/banco_carpintero/mesa_planos_ingenieria/mesa_ajedrez: opción `"Recoger huevo"`, mismo `"coger"` sin id. El nido en sí no "contiene" huevos (§11.3: el huevo sigue apareciendo junto al ave, el nido solo decide la cantidad) — es el punto de interacción natural que pidió el streamer, no una caja con inventario propio.
- **Cocinar el huevo — sin trabajo adicional**: `origenCocina:"animal"` ya hacía que un huevo cocinado al fuego diera `asado_huevo` y que entrara en la familia "Frito" en vasija — infraestructura genérica de cocina, no específica de huevo, verificada en el catálogo (`items/catalogo/items.json`) antes de esta pasada.
- **Verificado**: `tsc --noEmit` limpio en cliente y servidor tras los cambios, suite de servidor 1094/1094 sin regresión (esta pasada no tocó ningún archivo de `server/src`, solo `client/src` y catálogo). `client/test/objetosMundo.e2e.cjs` (nuevo, servidor Colyseus real, sin navegador): suelta un huevo real con el protocolo `"soltar"` YA existente (mismo `ObjetoMundoSchema` exacto que usan los huevos de granja), confirma que aparece en `state.objetosMundo` con `itemId`/`cantidad`/posición correctos, y que `"coger"` (el MISMO mensaje que manda la opción "Recoger huevo" del menú) lo retira de verdad — prueba el dato que consume el render nuevo de punta a punta. **No verificado con un navegador real** (mismo límite ya aceptado para barcos/monturas en su día): el render en sí es una caja placeholder sin lógica propia — dado el dato de entrada correcto (E2E de arriba) y que reusa un patrón ya visualmente verificado (`renderConstrucciones`), se documenta la certeza en el dato de origen en vez de gastar una pasada de Playwright en fotografiar una caja de color. **Tampoco se reprodujo un huevo real vía simulación de granja completa** (gallina+gallo domesticados, alimentados, madurados) — el mecanismo de PUESTA (§11.3) ya estaba probado antes de esta pasada; lo nuevo aquí es solo la mitad de cliente, probada con el mismo dato de entrada por la vía más corta.
