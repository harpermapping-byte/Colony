...
**ESTADO: AMPLIACIÓN IMPLEMENTADA Y VERIFICADA (2026-08-30)** sobre una base de Farycoins ya existente (Gremios/Mercado/Propiedades, ver GDD respectivos). Piezas nuevas: `server/src/datos/bd.ts` (`SALDO_INICIAL_JUGADOR`, `SALDO_INICIAL_NPC_COMERCIANTE`, `saldoInicialPara`, `creditarJarl` en `comprarOAlquilar`/`renovarTenencia`, `venderANpc`), `server/src/construccion/construccion.ts` (`nombresJarlTalCual`), `server/src/mercado/catalogoNpcComercio.ts` (nuevo, catálogo de tienda general), `server/src/mundo/agentes.ts` (`NpcBakeado.oficio`), `server/src/rooms/RegionRoom.ts` (`oficiosNpc`), `server/src/rooms/base/RoomExteriorBase.ts` (protocolo `npc:*` de comercio + loot de Farycoins al matar un enemigo). Probado: `economia.test.ts` (8 tests nuevos), `datos.test.ts`/`animalesGranja.test.ts` reajustados a los nuevos saldos, suite completa de servidor 636/636, `tsc --noEmit` limpio, `combate.e2e.mjs` en verde.
...
- **500 Farycoins por NPC comerciante** — con el catálogo de compra actual (1-2 Farycoins por unidad de recurso común), un NPC puede absorber entre 250 y 500 ventas de jugador antes de quedarse sin dinero (y entonces deja de comprar hasta que alguien le venda algo a ÉL... espera, no: hoy nada REPONE el saldo de un NPC salvo vender). **Esto es un sumidero neto en un solo sentido**: cada Farycoin que un NPC paga por comprar sale de su bolsa de 500 y nunca vuelve (no hay mecanismo de reabastecimiento). Con varios jugadores vendiendo activamente, un NPC concreto puede secarse en una sesión larga. Si ves que se vacían demasiado rápido, lo más simple es un tope de reposición diario (p.ej. +50/día de mundo, igual que el resto de mecánicas "una vez al día") — no lo implementé porque no lo pediste explícitamente, pero es la primera palanca que tocaría.
...
- **El saldo de un NPC comerciante nunca se repone** (§7) — puede secarse; a confirmar si quieres un tope de reposición diaria.
