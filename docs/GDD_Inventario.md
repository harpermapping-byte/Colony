...
### 10bis. Rejilla del cofre/arcón + intercambio real por drag&drop (pedido streamer 2026-09-06: "el arcón o cualquier objeto con inventario al darle click debe salir la opción abrir inventario y se abre la UI de su inventario con el grid y podrás dar a la I para abrir el tuyo e intercambiar objetos")

El clic sobre un `esContenedor` ("Abrir <nombre>" en el menú contextual → `cofre:consultar`) ya existía desde el 2026-08-31 — lo que faltaba era que `panelCofre.ts` mostrara una rejilla real (mostraba una lista plana de filas con botón "Sacar", sin drag&drop) y que se pudiera arrastrar CONTRA el propio inventario (tecla I, panel ya existente de §10).

**Servidor: cero cambio.** `cofre:meterItem`/`cofre:sacarItem` (`RoomExteriorBase.ts`) ya funcionaban desde antes de hoy y YA buscan hueco libre ellos solos (`buscarHueco`) — ni el cliente necesita calcular una celda de destino para este intercambio, a diferencia del drag&drop DENTRO del inventario propio (§10, que sí reordena a mano con `inventario:mover`). El único gap real era que nada en la UI los llamaba desde un gesto de arrastre.

**Cliente**: `panelCofre.ts` gana una `renderGrid()` — MISMO patrón exacto que `panelJugador.ts::renderGridContenedor` (celdas absolutas por `(x,y)`/`huella`, `draggable`) — y ambos paneles se coordinan por un payload JSON común en `dataTransfer` con un campo `origen: "jugador" | "cofre"`:
- Arrastrar una celda DESDE el inventario propio (`origen:"jugador"`) HASTA la rejilla del cofre → `cofre:meterItem`.
- Arrastrar una celda DESDE el cofre (`origen:"cofre"`) HASTA el inventario propio → `cofre:sacarItem` (el `ondrop` de `panelJugador.ts` detecta `origen:"cofre"` y lo enruta ahí en vez de a `inventario:mover`, que no aplicaría — el cofre no tiene mochilas/slots como el jugador, un único contenedor plano).
- Arrastrar DENTRO de la propia rejilla del cofre no hace nada (el cofre no reordena por `(x,y)`, no hay ganancia); el botón "Sacar" de siempre se mantiene igual de accesible que el arrastre.

`cofreObjetivo`/`panelCofre` (antes locales al bloque `if (SALA==="hub")`) se izaron a variables de módulo en `game.ts`, mismo patrón ya usado para `modoConstruccion`/`renderConstrucciones`/`panelAjedrez` — el `ondrop` de `panelJugador.ts` vive FUERA de ese bloque y necesita saber qué cofre está abierto ahora mismo.
...
## 12. Animación real de recoger/talar/picar + herramienta visible en la mano (pedido streamer 2026-09-06/07)

Pedido literal (dentro de un mensaje mucho más amplio sobre animaciones de todo el juego, priorizado por el streamer vía elección explícita — "Animaciones de acciones (recoger/talar/picar/golpear)" entre 4 bloques ofrecidos): *"si cojo una plantita, esa planta desaparece... luego talar o picar tiene que tener su animación (con el hacha y pico en la mano claro) y luego dará en el suelo madera piedra metales etc... las herramientas tenemos que ver que todas tengan uso y si falta algún uso por tener herramienta la añadimos"*.

**Coreografía (`client/src/render3d/rigHumanoide.ts`)** — función PURA `poseDeAccionHerramienta(tipo, progreso)`, sin THREE.js ni timers, para poder testear los ángulos exactos de forma determinista (`client/test/rigHumanoide.test.ts`, 9 tests):
- `"talar"`/`"picar"`/`"golpear"` comparten LA MISMA curva de golpe (armado hacia atrás 0→0.35, impacto 0.35→0.55, recuperación 0.55→1 — brazo derecho lleva el gesto, izquierdo acompaña al 30%, sin espejo porque no sujeta nada). Un solo golpe con el HACHA/PICO/ARMA equipada en la mano, no una animación por herramienta — coherente con que ninguna de las tres necesita una coreografía distinta (todas son "un golpe con algo en la mano").
- `"recoger"` es una curva DISTINTA: agacharse (piernas+torso+cadera bajando) y volver a subir, simétrica, sin gesto de brazo — se agacha a coger del suelo, no golpea nada.
- `RigHumanoide.actualizar` gana un parámetro final opcional `accion?: {tipo, progreso}` que aplica esta pose por encima de piernas/torso ya calculados por la marcha (mismo patrón que `tocando`/`trabajando`, aplicado el último para que gane).

**Disparo en red (broadcast, no solo al que actúa)**: `RoomExteriorBase.manejarCoger` (recoger/picar minería), `HubRoom`'s `arbol:talar` (talar) y `manejarCombateAccion` (golpear, solo si `atacante.esJugador`) emiten `this.broadcast("accion:jugador", {sessionId, tipo})` — así CUALQUIER cliente en la room ve la animación de cualquier jugador, no solo el propio. `client/game.ts` escucha ese mensaje, guarda `{tipo, inicio}` en el `EstadoJugador` correspondiente y el bucle de render calcula `progreso` con una duración fija por tipo (`DURACION_ACCION_MS`: recoger 550ms, talar/picar 750ms, golpear 350ms) hasta pasárselo a `rig.actualizar`.

**Herramienta visible en la mano, coherente con la acción — gap real de EQUIPO encontrado y cerrado de paso**: `mejorHerramientaPara` (`herramientasRecoleccion.ts`) escanea `contenedor.items` para decidir si el jugador puede talar/picar con nivel suficiente, pero **equipar un ítem lo QUITA de `contenedor.items`** (va a `player.inventario.equipo`, un `MapSchema` aparte) — así que un hacha/pico EQUIPADO (la única forma de que se vea de verdad en la mano vía `equipoVisual.ts`) era invisible para esa comprobación. Se cambió el criterio a mano en `arbol:talar` y en la rama de picapedrero de `manejarCoger`: ahora se exige el hacha/pico EQUIPADO en `manoPrincipal` (con familia/tier correctos), no simplemente presente en el inventario — cierra el pedido literal "con el hacha y pico en la mano claro" de raíz (antes se podía talar con el hacha en la mochila, sin verse en ningún sitio).

**GAP DE CONTENIDO real, grande, encontrado auditando el catálogo (pedido streamer: "ver que todas [las herramientas] tengan uso... si falta algún uso lo añadimos")**: de las ~65 herramientas de recolección/oficio del catálogo (`items/catalogo/items.json`, familias `herramienta_*` usadas por los 10 oficios craftables), solo 3 (`hacha_talar`, `pico_minero` tier 2, `azada_hierro`) tienen `prendaId` — el campo que activa el render en la mano vía `equipoVisual.ts::voxelesDePieza` (`if (!entrada?.prendaId) return [];`). Las otras ~62 (incluido `pico_minero_hierro`, tier 1, el pico "básico" real que un jugador nuevo craftea primero) tienen uso MECÁNICO real (gatean crafteo/recolección con normalidad) pero NUNCA se ven en la mano de nadie, se equipen o no. Esto es un hueco de CONTENIDO (arte vóxel + entrada `prendaId` por herramienta en `ropa/catalogo/prendas.json`, mismo mecanismo ya usado para armas/armaduras), no de mecánica ni de código — está fuera del alcance de esta pasada (que era la coreografía de animación + la corrección de gating), documentado aquí para una pasada de contenido futura. Mientras tanto, cualquier acción con una herramienta sin `prendaId` sigue jugándose con normalidad (golpe/talado/picado real, recurso real al suelo) solo que sin verse el hacha/pico de verdad en la mano.

Verificado: `client/test/rigHumanoide.test.ts` 9/9 (curva de pose determinista), suite completa de cliente 31/31, servidor 1216/1216, `tsc --noEmit` limpio en cliente y servidor, `server/test/herramientasRecoleccion.e2e.mjs` (actualizado para equipar el pico antes de picar, ya no basta con tenerlo en inventario) y `combateHabilidadDesgastePocion.e2e.mjs` sin regresión, y un E2E nuevo (`client/test/accionesAnimacion.e2e.mjs`, servidor+cliente+Playwright reales) que confirma de punta a punta: la planta desaparece al recogerla, el hacha se ve en la mano SOLO durante `talar` (no durante `recoger`), el pico se ve en la mano SOLO durante `picar`, y ambas herramientas requieren estar equipadas para funcionar. La medición exacta de ángulos de pose en el E2E resultó poco fiable bajo el renderer software de este entorno (SwiftShader) — se dejó como log informativo, la verificación de ángulos exactos vive en el test unitario puro.
