...
## 6bis. Reventa/oferta de inmueble entre jugadores (2026-09-03)

Gap real confirmado leyendo el código (mega-estrés 2026-09-02, ver CLAUDE.md): "NO existe reventa/oferta de inmueble entre dos jugadores (solo el jarl puede revocar y reasignar)". Cerrado.

**Decisión de diseño**: el DUEÑO decide, sin pasar por el jarl — a propósito el gate de "quién puede ofrecer" NO acepta `puedeActuarComoJarl` como atajo (a diferencia de otros gates de este documento). El jarl ya tiene su propio poder real sobre esto (`inmueble:revocar`, intacto); dejarle además vender la propiedad COMPRADA de otro jugador sin su consentimiento rompería el sentido de haberla comprado. Solo se puede ofrecer una propiedad en modo `compra` — un alquiler no es tuyo para revender. Sin comisión del jarl (mismo criterio que vender un objeto a otro jugador en Mercado — no toda transacción P2P paga al jarl, solo comprar/alquilar DEL asentamiento).

**Tabla nueva** `ofertas_inmueble` (`propiedad_id, ofertado_por, destinatario, precio_farycoins, creado_en`, PK por `(propiedad_id, destinatario)`) — el dueño puede tener varias ofertas simultáneas a distintos jugadores sobre la misma propiedad (decide quién compra primero); vive aparte de `propiedades`, una oferta puede caducar/cancelarse sin tocar la propiedad en sí.

**Protocolo Colyseus** (RegionRoom, junto a `inmueble:*` de §5):
- `"inmueble:ofrecer" { inmuebleId, jugadorNombre, precioFarycoins }` — solo el dueño actual. Empuja `client.send("inmueble:ofertaRecibida", {propiedadId, ofertadoPor, precioFarycoins})` al destinatario SI está conectado a esta room ahora mismo; si no, la oferta sigue en BD y la recupera con `misOfertas` al conectar (a diferencia de las invitaciones de Gremios, esto SÍ es recuperable offline).
- `"inmueble:cancelarOferta" { propiedadId, jugadorNombre }` — solo quien hizo la oferta.
- `"inmueble:aceptarOferta" { propiedadId }` — solo el destinatario. Todo o nada (`bd.transferirPropiedad`, mismo compare-and-swap `WHERE dueno = ?` que el resto de mutaciones económicas de `bd.ts`): cobra al comprador, transfiere el dueño SOLO si el vendedor sigue siéndolo en ese instante (si perdió la carrera — revocada por el jarl, vendida a otra oferta ya aceptada — reembolsa y falla limpio), abona al vendedor, y limpia TODAS las demás ofertas pendientes sobre esa propiedad (ya no aplican con el dueño nuevo). Éxito → `this.broadcast("inmueble:actualizado", ...)` si la propiedad es de ESTA región + `inmueble:ofertaAceptada` directo al vendedor si sigue conectado (puede ser de otra región/desconectado).
- `"inmueble:misOfertas" {}` → `client.send("inmueble:misOfertas", {recibidas, hechas})` — ambas listas con el NOMBRE del otro jugador ya resuelto (no solo su id).

Nota: `propiedadId` en `cancelarOferta`/`aceptarOferta`/`misOfertas` es el id COMPLETO (`i_<mapaId>:<edificio>`), a diferencia del `inmuebleId` corto que usa el resto de mensajes `inmueble:*` — una oferta puede referirse a una propiedad de OTRO asentamiento, no solo del mapa donde estás conectado ahora.

Verificado con servidor real (2 sesiones colyseus.js, sin cliente): compra fresca → oferta → `misOfertas` recibida (con el saldo insuficiente rechazado correctamente) → oferta aceptada con fondos → dueño y Farycoins transferidos de verdad, broadcast a ambos jugadores → doble-aceptar tras completarse falla limpio → el vendedor original ya no puede reofrecer (ya no es dueño). 9 tests nuevos en `server/test/reventaInmueble.test.ts` (upsert de oferta, resolución de nombres, transferencia feliz sin comisión del jarl, limpieza de ofertas huérfanas, pérdida de carrera con reembolso, fondos insuficientes, alquiler no revendible, auto-compra rechazada). 1181/1181 tests del servidor.
