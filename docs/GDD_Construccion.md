...
Pedido del streamer (al irse a dormir, "siéntete libre"): *"que el carpintero pudiera hacer muchos muebles para decorar las casas, y estantes o estanterías para objetos, por ejemplo estanterías de poción, que se vea las pociones al colocarla, son inventarios más limitados pero visuales, candelabros, lámparas de aceite, expositor de armas o armaduras, armario para ropa, distintos sofás de distintos materiales... de 1 o 2 plazas, camas de diferentes tiers de material y estéticas, de 1 o 2 personas... si hago caja, pues tiene inventario, si hago cajón tiene inventario"*.

### 9.1 Qué había y qué faltaba de verdad

- Colocar un mueble de `elementos.json` era GRATIS para cualquiera con parcela (§3, "sin receta = gratis por ahora") salvo las ~60 piezas exclusivas con `requiereItemColocar` (docs/GDD_Profesiones.md). El carpintero tenía 11 muebles decorativos exclusivos (§11 de docs/GDD_Crafteo.md) y ninguno hacía nada: ni cama, ni asiento, ni contenedor.
- Un contenedor (`esContenedor`) era siempre un cofre genérico: aceptaba cualquier ítem, su rejilla salía de `sqrt(aportes.almacenamiento)` y su contenido solo lo veía el dueño al abrirlo — nada se veía sobre el mueble.
- Sentarse por clic (`sentar:iniciar`) no comprobaba ocupación; sentarse por tecla F (`asiento:sentarse`) bloqueaba a 1 por mueble sin excepción; dormir no comprobaba ocupación ("gap preexistente" documentado en el propio código) y todas las camas daban el mismo buff.

### 9.2 Contrato nuevo (campos ADITIVOS de `interiores/catalogo/elementos.json`, todos opcionales)

| Campo | Consumidor | Semántica |
|---|---|---|
| `plazas` | `mobiliario.ts::plazasDe` + `sentar:iniciar` / `asiento:sentarse` / `dormir:iniciar` | Cuántos jugadores caben A LA VEZ. Ausente = 1 (comportamiento previo). Cuenta a los ocupantes por los DOS mecanismos de sentarse (clic y tecla F escriben `Player.sentado`/`sentadoEnId`, el Schema es la única fuente que los ve juntos). Camas: `Player.durmiendo`/`durmiendoEnId`. |
| `calidadDescanso` | `mobiliario.ts::factorDescansoDe` + `dormir:completar` | Multiplica la duración del buff "descansado" (`xpOficioX2`, docs/GDD_Mecanicas.md §5.12): pino 1.0, roble 1.25, abedul tallado 1.5, noble con dosel 2.0, `cama_dosel_maestra` 2.2, litera 0.9. Acotado 0.25-4. `dormir:completado` devuelve `factorDescanso`. |
| `aceptaItems` `{tipos?, slots?, prefijos?, ids?, etiqueta}` | `mobiliario.ts::aceptaItemEnMueble` + `cofre:meterItem` + pista en `panelCofre.ts` | Filtro de contenido: basta cumplir CUALQUIERA de las listas (`tipos` de catálogo, `slots` de equipo, `prefijos` del itemId, `ids` exactos). Rechazo: `cofre:error "ese mueble solo guarda <etiqueta>"` (toast en cliente). Sin filtro = cofre de siempre. Las entregas de transporte (docs/GDD_Produccion.md §3ter) NO pasan el filtro a propósito: son contratos ya validados por el jugador al firmarlos. |
| `rejillaCofre` `[ancho, alto]` | `capacidadCofre` | Rejilla EXACTA del contenedor; manda sobre `libreria.capacidad` y sobre la raíz cuadrada de `aportes.almacenamiento`. Una estantería de pociones es `[6,1]`: seis frascos en fila, los mismos seis que se ven encima. |
| `expositor` | `construccion:nueva` / `construcciones:lista` (campo `expuestos`) + broadcast `construccion:expuestos {id, expuestos}` en `guardarContenedorDeCofre` y en la entrega de un transporte | El contenido se DIBUJA sobre el mueble para TODOS (`renderConstrucciones.ts::actualizarExpuestos`: un prop pequeño por ítem, del `colorDebug` del ítem, más alto si el ítem es alargado — armas de pie —, en filas sobre la tapa real del placeholder o del `.glb` cuando carga). Tope `MAX_EXPUESTOS=24`. Un cofre normal no difunde nada: su contenido sigue siendo privado. |
| `capa:"iluminacion"` (ya existía) | `EntradaConstruible.iluminacion` → `renderConstrucciones.ts::encenderLuz` | Cualquier lámpara/candelabro/farol colocado por un jugador enciende una `PointLight` cálida real (misma tonalidad que las lámparas bakeadas de `interiorVisual.ts`), sin sombras, a 1.4u — pocas por parcela, nunca una por prop del mapa. |

Los 5 campos están documentados también en `elementos.json::_camposConsumidores`. Ninguno cambia el bake de NPCs: las piezas nuevas llevan `requiereItemColocar`, que `colocarElementos.js` ya filtra (test "requiereItemColocar NUNCA sale del bake").

### 9.3 Catálogo: 88 piezas nuevas (84 carpintero + 4 curtidor) + 2 ítems enlazados + 12 entradas existentes que ganan semántica

Generadas con un script de una sola pasada (nombre vía `nombreBonito.js`, `valorBase`/`xpOtorgada` vía `node items/catalogo/valorBase.js --aplicar`), cada una = entrada en `elementos.json` (`requiereItemColocar` a sí misma, `tiposSalaValidos:["taller"]`) + ítem portador `tipo:"recurso"` en `items.json` + receta `<id>_craft` (mesas y tiempos dentro de la banda YA existente de cada nivel, `T={1:18,2:25,3:30,4:35,5:45,6:55,7:65,8:85}`):

- **Camas** (`esCama`, `plazas`, `calidadDescanso`): pino individual/doble (n1-2), litera de pino (2 plazas, 0.9), sauce y mimbre (n3), roble individual/doble (n3-4, 1.25), abedul tallada individual/doble (n5-6, 1.5), noble doble con dosel (n7, 2.0), diván de seda noble (n8: 2 plazas sentado + siesta 0.8), cuna de abedul (decorativa).
- **Asientos** (`esSilla`+`esAsiento`+`plazas`): taburete y silla de pino (n1), banco de roble doble (n2), butaca de lino (n2), sofá de lino doble/triple (n3-4), banco de abedul con respaldo, butaca de cuero y sofá de cuero doble/triple (n4-5), silla de abedul tallada y mecedora de sauce (n5), butaca/sofá doble/triple de terciopelo noble (n6-7), sillón orejero de cuero (n7). `sofa`/`banco` del catálogo viejo ganan `plazas:2`.
- **Contenedores** (`esContenedor`+`rejillaCofre`): caja de pino 3x2, cesto de mimbre 3x3, baúl de pino 4x3, cajón de roble 3x3, cómoda de pino 4x3, mesitas de noche con cajón, armario de pino 5x5, baúl de roble con herrajes 6x3, cómoda barnizada y aparador 6x4/6x3, arcón tratado reforzado 7x4, armario grande de roble 8x6, baúl noble tallado 8x4, escritorio de roble con cajón, tocador con espejo.
- **Contenedores con filtro**: alacena de cocina y despensa (comida/provisiones), caja de herramientas y panel de herramientas (herramientas), cajón de semillas, armario de ropa de abedul y ropero noble con espejo (`tipos:["equipable"]`), `armario_nogal_barnizado` (existente, ahora ropero real).
- **Expositores** (`expositor:true` + filtro + rejilla visible): estantería de pino para pociones 6x1 y doble de roble 8x2, platero 5x2 (vajilla), botellero 4x2 (bebidas; `tonel_hidromiel_gremial` existente también), expositor de armas de pared 3x3 y grande 6x3, armero de pie 4x3, maniquí para armadura y maniquí noble 2x3 (`tipos:["armadura"]`), vitrina de cristal para joyas 4x2 (`slots` anillo/cuello/brazalete), vitrina de abedul para libros 6x2, repisa de trofeos 3x2, perchero de pie 3x1 (capas/sombreros/abrigos), estante de pared 4x1 y estanterías abiertas de roble/abedul (sin filtro: todo lo que dejes encima se ve). Los bake-only `estanteria_pociones`/`expositor_armas`/`vitrina_joyas`/`estante_herramientas` ganan la misma semántica (siguen siendo gratis, §3).
- **Iluminación** (`capa:"iluminacion"`): candelero de mesa de pino, candelabro de tres brazos de madera, lámpara de aceite de mesa de cobre, candelabro de pie de hierro, farol colgante de cobre, lámpara de aceite de pie de hierro, candelabro de mesa de cobre, araña de cobre de seis velas; `lampara_vidrio_coloreado` (ítem del joyero que ya existía con receta pero sin mueble) ahora se coloca y alumbra.
- **Mesas/decoración**: macetero, mesa redonda de pino, mesa de centro de abedul, mesa de banquete noble (n8), cuadros con marco de pino/roble, biombo pintado, espejos con marco y de pie, reloj de pared. **Curtidor** (telar): alfombras de lana pequeña/grande (`FLOOR_DECAL`, pisables), cortina de lino, tapiz de lana.
- Enlazado de paso: `barril_roble` (ítem con receta de carpintero que no tenía mueble) ahora es contenedor 3x3 colocable.

### 9.4 Servidor y cliente

- `server/src/construccion/mobiliario.ts` (nuevo, puro): `aceptaItemEnMueble`, `expuestosDe` (orden de rejilla estable, un montón cuenta una vez), `plazasDe`, `factorDescansoDe`. `catalogo.ts` propaga los campos + `iluminacion` derivado de `capa`.
- `RoomExteriorBase.ts`: `asientosOcupados` pasa de `Map<id, sessionId>` a `Map<id, Set<sessionId>>`; `ocupantesDeMueble`/`durmientesEnCama` cuentan sobre el Schema; `manejarSentarIniciar`/`manejarAsientoSentarse`/`manejarDormirIniciar` rechazan al que no cabe ("no queda sitio en ese mueble" / "ese asiento ya está ocupado" / "esa cama ya está ocupada"); `manejarDormirCompletar` lee la cama ANTES de soltar `durmiendoEnId`; `difundirExpuestosSiExpositor` desde `guardarContenedorDeCofre` y desde la entrega de transporte.
- Cliente: `catalogoConstruccion.ts` (`iluminacion`/`expositor`/`plazas`/`aceptaItemsEtiqueta`/`requiereItemColocar`), `renderConstrucciones.ts` (luces y props de expuestos viven APARTE de la malla del mueble, que se sustituye por el `.glb` real al cargar — se recolocan sobre la tapa nueva), `game.ts` (`construccion:expuestos`; `cofre:error`/`sentar:error`/`asiento:error` ya se ven como toast, antes solo consola), `panelCofre.ts` (línea "Solo guarda: …" bajo el título).
- Arte: `taller-vox/generar_modelos.js` clasifica sofá/butaca/sillón/diván como asiento TAPIZADO (bloque + cojín por plaza + reposabrazos + capitoné), cómoda/alacena/despensa/vitrina/botellero/expositor/aparador/estante como contenedor alto (abierto si es estantería/expositor/vitrina/botellero), caja/cajón/cesto/tonel como contenedor bajo, y formas propias por palabra clave (candelabro, candelero, lámpara de aceite, farol, maniquí, cuadro, tapiz, cortina, biombo) para objetos de pie y de mesa; camas con dosel y literas con estructura real. **Alturas recalibradas de paso** (mismo bug "muebles gigantes" de docs/GDD_Motor_3D_Props.md, que el 2026-09-06 solo corrigió silla/mesa/cama): armario/estantería 3.2→1.9 casillas, cómoda/aparador 0.95, arcón/baúl 1.3→0.8, caja/cesto 0.55, objetos de pie (perchero/maniquí/armero/candelabro de pie/espejo de pie) 0.7→1.5, araña y farol colgante flotan a la altura del techo. Regenerados y exportados con `--centrar-xz` (216 `.glb` existentes cambian por las alturas, 110 nuevos; el resto byte a byte idéntico — determinismo confirmado con `silla_01.glb`).

### 9.5 Verificación

- `server/test/mobiliario.test.ts` (5): filtros por tipo/slot/prefijo/id, orden y tope de `expuestosDe`, acotado de plazas/calidad, y una pasada sobre el catálogo REAL (todo filtro acepta al menos un ítem real y su rejilla lo aloja; todo expositor es contenedor; `plazas` solo en asientos/camas; lámparas con `iluminacion`; el carpintero nunca coloca gratis).
- `server/test/mueblesCarpintero.e2e.mjs` (24/24, colyseus.js puro, 4 jugadores sobre `testflat`): sin ítem no se coloca; con ítem se coloca, se consume y `construccion:nueva` lleva `expuestos:[]` a todos; rejilla 6x1; meter poción → `construccion:expuestos` en los otros clientes; meter espada → rechazo por filtro; sofá doble: clic + tecla F caben, el tercero es rechazado por los dos mecanismos, al levantarse uno el tercero entra; cama individual rechaza al segundo, `dormir:completado` trae `factorDescanso:1.25`; un cliente que entra después recibe la estantería con sus expuestos persistidos en BD. Lección del arnés: el servidor SIEMBRA la Test Zone Norte (`semillaTestZone.ts`, x:28-34 y:12-18) incluso con BD fresca — las casillas del e2e quedan fuera de esa franja.
- `taller-vox/test_muebles_proporcion.js` (+4): alturas de armarios/cómodas/baúles, sofás como asiento con el triple más ancho que el doble, objetos de pie a la altura de una persona y piezas colgantes sin tocar el suelo, ninguna pieza nueva cae al arquetipo genérico.
- Sin verificar en el hosting real del streamer (el render de props/luces se probó con Playwright en este sandbox sin GPU — `client/test/mueblesVisual.e2e.mjs`).

### 9.6 Pendiente real

- ~~El panel de construcción (tecla B) sigue listando los ~800 muebles del catálogo sin indicar cuáles exigen ítem ni cuáles tiene el jugador~~ **RESUELTO, ver §9.8.**
- ~~Los props expuestos son cajas de color por ítem (nunca el `.glb` del ítem)~~ **RESUELTO, ver §9.7.**
- Un mueble tallado por el carpintero legendario sigue sin heredar estos campos (mismo límite de §8).

### 9.7 Expositores con el modelo real de cada ítem (2026-09-11, misma noche — "este camino me gusta")

El streamer preguntó cómo cerrar los dos pendientes de §9.6; para el segundo eligió reusar arte existente en vez de un pipeline nuevo. Lo que hay:

- **Fuente del modelo por categoría de ítem**: `tipo:"arma"` → `assets/armas/<id>_01.glb` (ya existían, 2026-09-08), `tipo:"herramienta"` → `assets/herramientas/<id>_01.glb` (ídem), y TODO lo demás → `assets/objetos/<id>_01.glb` — categoría NUEVA de `CategoriaAsset` (`client/src/render3d/assetCatalog.ts`), servida con la misma caché que el resto del arte (`servidorEstatico.ts::CATEGORIAS_ARTE` + `client/vercel.json`).
- **Lote `assets/objetos/` (138 `.glb`, ~0.9MB)**: `taller-vox/generar_objetos.js` YA existía (75 ids `tipo:"objeto"`: vajilla, libros, catalejo…, documentado en `docs/GDD_Motor_3D_Props.md`) pero nunca se había exportado. Ahora además genera lo que un jugador deja A LA VISTA en un expositor (`IDS_EXPOSITOR`, 87 ids): pociones/elixires/tónicos (arquetipo nuevo `FRASCO`: cuerpo del color del líquido + cuello de cristal + corcho), joyas (`JOYA`: aro hueco con engaste para anillo/brazalete, óvalo con colgante para collar), piezas de armadura (`ARMADURA`: cúpula para casco, torso, dos columnas para piernas, dos bloques para manos/botas) y libros (arquetipo `PLANO` de siempre). Exportado con `node exportar_lote.js objetos_generados.json ../assets/objetos` (SIN `--centrar-xz`: el cliente centra por caja delimitadora, ver abajo). Mismo "enganche rápido" que el resto de lotes de la sesión — arte placeholder vóxel, sin revisión pieza a pieza.
- **Cliente** (`renderConstrucciones.ts::actualizarExpuestos` → `sustituirPropExpuesto`): cada ítem sale al instante como la caja de color de §9.4 y, en cuanto `obtenerPlantilla(categoria, itemId)` resuelve, se sustituye por un clon del `.glb` escalado por su caja delimitadora al hueco que le toca sobre la tapa (ancho/fondo ≤ 0.3 casillas; alto ≤ 0.28, o 0.6 si el ítem es alargado — armas/herramientas quedan "de pie", el resto tumbado/pequeño), centrado en XZ y apoyado por su base. Sin `.glb` para ese id, la caja se queda para siempre: nunca se rompe nada. Cuidados reales: (1) **versión por refresco** — un `.glb` que termina de cargar DESPUÉS de que el expositor se haya redibujado (otro ítem metido/sacado, o el mueble sustituido por su propio modelo) se descarta, nunca se cuela en un grupo viejo; (2) **dispose selectivo** — solo se disponen las cajas propias (`userData.propioDelSector`), un clon del `.glb` comparte geometría/material con la plantilla cacheada de `entityLoader` y disponerlo rompería ese modelo para cualquier otro expositor.
- **Verificación**: `client/test/mueblesVisual.e2e.mjs` 17/17 (nueva comprobación: tras arrastrar las dos pociones, la sonda `expuestosVisibles(id).conModelo` llega a 2 — las dos cajas se cambiaron por el frasco real); `client/test/verGlbAislado.html?url=/assets/objetos/<id>_01.glb&encuadre=auto` (parámetro nuevo del visor genérico: encuadra la cámara a la caja del modelo, imprescindible para piezas de 0.3 unidades) con capturas de frasco/anillo/casco/vaso/libro en `client/test/capturas/objeto_*.png`. Cliente 78/78, `tsc --noEmit` limpio en cliente y servidor.
- **Pendiente**: sin verificar en el hosting real del streamer. El primer punto de §9.6 (panel de construcción) se cerró a continuación, §9.8.

### 9.8 Panel de construcción (tecla B): qué exige ítem, qué tienes, pestañas y búsqueda (2026-09-11, misma noche — "adelante")

Cierra el primer pendiente de §9.6. Hasta ahora el panel de la tecla B (`client/src/construccion/constructor.ts`) era una lista plana de ~800 botones con el id crudo (`estanteria_pociones_pino`), sin saber nada del inventario: para enterarse de que un mueble del carpintero cuesta un ítem había que intentarlo y leer `construir:error`. Lo que hay ahora:

- **Tres estados por construible, calculados contra el inventario replicado** (`EstadoConstruible`): `gratis` (ni `requiereItemColocar` ni `receta` — la silla de siempre), `tienes` (el ítem está en el inventario, insignia verde `✓ N`; o todos los materiales de la `receta` de una mesa de oficio, `✓ materiales`) y `falta` (`🔒 ítem` / `🔒 materiales`, botón atenuado). El conteo sale de un callback `contarItem(itemId)` que `game.ts` resuelve sobre `players[yo].inventario.cuerpo.items` — el MISMO contenedor donde el servidor busca `requiereItemColocar`/`receta` al colocar, así lo que el panel marca coincide con lo que el servidor aceptaría. Puramente informativo: la verdad sigue siendo `construir:error`.
- **Tooltip útil, no solo el veredicto**: para un mueble que exige ítem, "Se consume 1× Estantería de Pino para Pociones al colocarlo (tienes 1). Lo craftea el carpintero (nivel 2) en Banco de Carpintero." — la mesa y el oficio salen de `items/catalogo/recetas.json` (buscando la receta cuyo `resultado.itemId` es ese ítem; el JSON ya estaba en el bundle por `panelCrafteo.ts`, coste cero). Para una mesa con `receta`: la lista completa de materiales y, si falta algo, "Te falta: 9× Madera Blanda, …" con el déficit exacto. El "Requiere nivel N de oficio" que ya existía se conserva delante.
- **Refresco en caliente**: `refrescarDisponibilidad()` se llama al abrir el modo y, mientras está abierto, con cada `onAdd`/`onRemove` del array del inventario (el mismo hook que refresca la rejilla del panel del jugador, §9) — colocar la estantería consume el ítem y su fila pasa a `falta` sin cerrar el panel.
- **Pestañas** `Todo / Puedo colocar / Me falta / Gratis` (`filtrar()`) y **búsqueda** por nombre o id (`data-testid="construccion-busqueda"`, con `stopPropagation` en su `keydown` para que escribir una "r" no rote el fantasma). Las cabeceras de categoría muestran `visibles/total` y desaparecen cuando no queda nada. Los botones muestran el **nombre bonito** (`docs/GDD_Inventario.md` §11) en vez del id; el id sigue en `data-id` y en el tooltip del cuadradito de color.
- **Quirk preexistente que salió a la luz**: `maceta_pequena`/`maceta_grande` existen a la vez en `elementos.json` (mueble) y `exteriores.json` (exterior) — dos botones legítimos con el mismo id, y el `Map` `botones` por id ya dejaba uno sin resaltar al seleccionar. Las filas nuevas van por clave `categoria:id`; `estadoDe(id)` prefiere el mueble.
- **Verificación**: `client/test/mueblesVisual.e2e.mjs` 29/29 (12 comprobaciones nuevas: B abre el modo tras asignarse `p_0001`, estantería `tienes ✓ 1` con el tooltip del carpintero, `cama_pino_individual` `falta 🔒 ítem`, `silla` gratis sin insignia, `banco_carpintero` `🔒 materiales` con "Te falta:", nombre bonito en el botón, pestaña "Puedo colocar" 667 visibles / "Me falta" 233, búsqueda "pociones" → 3 estanterías, Escape cierra, y tras colocar la estantería su fila pasa a `falta`), captura `client/test/capturas/muebles_panel_construccion.png`. Cliente 78/78, `tsc --noEmit` limpio. Sin cambio de servidor ni de protocolo.
