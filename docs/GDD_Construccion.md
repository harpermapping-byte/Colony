...
- **Fuera de alcance de este cambio**: parcelas "especial" (proyectos comunales del jarl, 14 listados en `docs/Backlog_Mecanicas_Futuras.md`) no tienen todavía ningún tratamiento distinto de las "normal" — el `tipo` viaja en el id pero nada lo consume aún. Ninguna región DISTINTA de la capital tiene `parcelasReservadas` hoy (decisión de bake, no de código: cualquier tier que el streamer quiera abrir a construcción de jugador solo necesita que `ciudades/` le añada ese campo).
