...
- UI de personaje (barras de vitales, panel de atributos, icono de "necesitas cagar" a partir del 75% de `caca`, animación de `higiene:cagar`) — "lo último", pedido explícito del streamer, junto con el resto de interfaces.
- **Clima y estaciones** (pedido explícito 2026-08-30, mismo mensaje que §3.6, FUERA de esta pasada por tamaño — se documenta aparte cuando se implemente): calendario de 12 meses de 30 días/4 estaciones de 3 meses (`assets/mundo/tiempo.json` tiene HOY `diasPorEstacion:7`, placeholder de la primera pasada de `tiempoMundo.ts` — cambiar a 90 y añadir `mes` es la primera pieza), filtro de color sutil por estación en el cliente (verano más luminoso, otoño beige, invierno blanco, primavera azulado/verde), estados de clima (lluvia/nieve/viento/sol/nublado, sin nieve acumulable todavía — pedido explícitamente aparte), temperatura de mundo por estación, y regulación de temperatura corporal del jugador (ropa con `quitaFrio`/`quitaCalor`, sin declarar todavía en `ropa/catalogo/`; alejarse del rango cómodo baja Resistencia y acelera el gasto de comida/bebida). Diseño grande, mismo criterio "proponer primero, confirmar, implementar" que el resto de sistemas de esta envergadura (ver GDD_Combate.md §9 como precedente).
