...
## 18. Cocina v3 (2026-09-01) — minijuego real-time reusando estacionFuego.ts

Pedido literal del streamer: *"dale con minijuego cocina"* — mismo sistema de activarse que herrería/pociones. Decisión de alcance explícita, elegida por el streamer entre varias opciones (una sola vasija "avanzada" vs. todas): **"todas las vasijas"** — las 6 piezas `esVasija:true` (`cuenco_cocina`, `cazuela_cocina`, `olla_cocina`, `cuenco_barro_grande`, `olla_grande`, `tinaja_batidos`) pasan a exigir **cocinero nivel 2** y a resolverse con la sesión interactiva; `cocina:simple` (hoguera/chimenea, sin vasija) y ensalada/bocadillo/cortarPan (cortar/montar sin fuego, no "cocinar" de verdad) se quedan **exactamente igual que siempre** — sin gate, sin minijuego, sigue siendo la vía rápida/accesible para quien todavía no tiene nivel 2.

### 18.1 Motor — mismo estacionFuego.ts que alquimia, cero código nuevo de temperatura

`server/src/cocina/cocina.ts` gana una sesión (`SesionCocina{estacion, resultadoBase, familia, itemIdsIngredientes, bonusCantidadPocion}`) que envuelve `estacionFuego.ts` EXACTAMENTE como `alquimia.ts` — `iniciarSesionCocina` congela `cocinarPlato(ingredientes, capacidadMax)` DE UNA VEZ al arrancar (mismo criterio "los ingredientes deciden QUÉ sale, el fuego solo escala CUÁNTO"); `avivarCocina`/`enfriarCocina` delegan tal cual; `servirCocina` escala `vida`/`estamina`/`comida`/`bebida` del resultado congelado por el factor de pureza (`FACTOR_PUREZA_MINIMO_COCINA=0.4` a 1.0, misma curva y mismo suelo que `FACTOR_PUREZA_MINIMO` de alquimia — constante PROPIA, no importada, para no acoplar `cocina.ts` a `construccion/alquimia.ts`) — **`platos`/`mezclaBonus` NUNCA se escalan**: cuántas raciones salen y si hubo bonus de mezcla ya lo decidieron los ingredientes, el fuego solo afecta la calidad de cada ración.

`CONFIG_ESTACION_COCINA` usa los mismos números de partida que `CONFIG_ESTACION_ALQUIMIA` (ventana 55-80°, `duracionMinimaSeg:8`) — mismo criterio de placeholder de balance a afinar, sin motivo para que cocina y alquimia difieran todavía.

### 18.2 Protocolo (`RoomExteriorBase.ts`)

`cocina:llenarAgua`/`cocina:anadir` **sin cambios** — la vasija sigue acumulando ingredientes exactamente igual que en v1/v2. Lo que cambia es el verbo final:

- `cocina:preparar {construccionId}` — ya NO es instantáneo: valida cocinero nivel 2 (`entrada.nivelOficioMinimo`, mismo mecanismo que `caldero` en pociones — leído del propio catálogo, nunca duplicado a mano), valida que no haya otro minijuego/crafteo en curso, congela `cocinarPlato` sobre lo que había en la vasija (`iniciarSesionCocina`), vacía la vasija YA (los ingredientes ya se gastaron del inventario en `cocina:anadir`, esto solo resetea el estado de la construcción) → `cocina:iniciado {construccionId, cfg, sesion}`.
- `cocina:accion {accion:"avivar"|"enfriar"}` → `cocina:progreso {sesion}`.
- `cocina:servir` → resuelve pureza, registra/reusa el plato (MISMO `clavePlato`/`nombrePlato`/`platos_creados` de siempre, sin tocar esa mecánica), entrega con `entregarOSoltar`, otorga XP de cocinero + inteligencia (mismo patrón que `manejarAlquimiaColar`: XP de oficio solo si `cocinero` está elegido en uno de los 2 slots, inteligencia siempre) → `cocina:preparado {itemId, nombre, cantidad, mezclaBonus, pureza, oficio, xp, nivel, enSuelo}` — **mismo nombre de evento que v1/v2**, solo que ahora llega al final de la sesión en vez de al instante.
- `cocina:cancelar` → `cocina:cancelado` (los ingredientes NO se devuelven, ya se gastaron en `cocina:anadir`).

Igual que forja/alquimia: movimiento bloqueado mientras `cocinasEnCurso.has(sessionId)`, limpieza en `onLeave`.

### 18.3 Fallo real encontrado y corregido: el guard "ya tienes X en curso" no era exhaustivo

Al añadir el cuarto sistema (`cocinasEnCurso`) se auditaron los 3 guards existentes: `manejarCrafteoIniciar` solo comprobaba `craftesEnCurso`/`forjasEnCurso`, **nunca `alquimiasEnCurso`** — un jugador con una poción en el caldero podía arrancar un crafteo normal a la vez (bug real preexistente, no introducido por esta pasada, pero sí encontrado por ella). Corregido centralizando en `algunMinijuegoEnCurso(sessionId)` — un único punto de verdad que comprueba las 4 listas, usado ahora por los 4 guards. Efecto colateral aceptado: el mensaje de error pasó de ser específico ("ya tienes una forja en curso") a genérico ("ya tienes un crafteo o minijuego en curso") — más simple, un jugador no necesita saber CUÁL de los 4 sistemas tiene abierto para saber que no puede abrir otro.

### 18.4 Reutilización de las pociones (docs/GDD_Pociones.md, ampliación 2026-09-01)

- **`xpOficioX2`** — envuelve el `delta` de `bd.sumarXpOficio(..., "cocinero", ...)` en `cocina:servir`, mismo `xpConBuffPocion` que ya usan los otros 7 sitios.
- **`produccionCrafteoX2`** — `bonusCantidadPocion` se congela en `cocina:preparar` (mismo momento que `bonusCantidad`/`bonusCantidadOficio` de crafteo) y se aplica a `resultado.platos` en `cocina:servir` (`cantidadFinal = floor(platos × (1+bonus))`). Amplía el alcance documentado en GDD_Pociones.md §1.4 (antes solo crafteo.ts) — las raciones de un plato de vasija encajan exactamente en "resultado.cantidad de un crafteo", a diferencia de herrería (pieza única) o la propia poción (instancia única con tirada propia).
- **`sigilo`/`vida`/`velocidad`/`estamina`/`carga`** no tocan cocina — no hay ningún punto de esta mecánica que les corresponda.

### 18.5 Cliente — panel YA existente extendido, no uno nuevo

A diferencia de herrería/pociones (sin panel todavía), cocina SÍ tenía un panel real de testeo (`client/src/cocina/panelCocina.ts`, v1). Se extendió con `mostrarSesion`/`actualizarSesion`/`mostrarResultado`/`ocultarSesion` (mismo criterio de placeholder — texto plano y botones, sin barras ni escena 3D, a diferencia de `panelForja.ts`) en vez de crear un panel aparte: el botón "Preparar plato" ahora arranca la sesión, que sustituye la vista normal de la vasija hasta servir/cancelar.

### 18.6 Verificado

- `server/test/cocina.test.ts` ampliado de 18 a 32 tests (14 nuevos: congelado al iniciar, `servirCocina` con pureza casi perfecta/pésima, `platos`/`mezclaBonus` nunca escalados, delegación de avivar/enfriar).
- `server/test/cocina.e2e.mjs` (NUEVO) — protocolo real contra el servidor: allowlist de ingredientes (rechaza `lingote_hierro`), gate de cocinero nivel 2, guard mutuo (doble `cocina:preparar` rechazado), servir antes de tiempo rechazado, sesión completa con `cuenco_barro_grande` (sartén, sin hervor) dando `mezclaBonus` real con zanahoria+carne_roja, XP/nivel de cocinero real, cancelado sin devolución.
- `herreria.e2e.mjs`/`alquimia.e2e.mjs`/suite completa (1074 tests) re-verificados tras el guard unificado — un solo ajuste de texto esperado en `herreria.e2e.mjs` (mensaje de error genérico nuevo), sin regresión funcional.
- `tsc --noEmit` limpio en `server/` y `client/`.

### 18.7 Pendiente

- Sin distinción visual del minijuego por vasija (todas usan el mismo `CONFIG_ESTACION_COCINA`) — si algún día se quiere que la olla grande sea "más difícil" que el cuenco pequeño, es un ajuste de config por vasija, no de mecanismo.
- El resto de huecos honestos de v2 (§16) siguen abiertos igual — quesera, tipos de leche, etc.
