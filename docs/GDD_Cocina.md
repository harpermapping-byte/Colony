...
**ESTADO: v1 IMPLEMENTADA Y VERIFICADA (2026-08-30).** Piezas: `server/src/cocina/cocina.ts` (nuevo, puro: boost simple, cocción de vasija, clave/nombre de plato), `server/src/inventario/inventario.ts` (+`AportesCocina`, `aportesCocina`/`origenCocina`/`restauraMultiple` en `EntradaCatalogoItem`), `server/src/datos/bd.ts` (tabla `platos_creados`, dual SQLite/Postgres), `server/src/construccion/catalogo.ts` (+campo `cocina`), `server/src/rooms/base/RoomExteriorBase.ts` (mensajes `cocina:*`, `manejarPersonajeConsumir` extendido a varios vitales a la vez), `items/catalogo/items.json` (16 ingredientes existentes con `aportesCocina`/`origenCocina` + 16 versiones `_cocinado` nuevas), `interiores/catalogo/exteriores.json` (`hoguera_campamento` + `cuenco_cocina`/`cazuela_cocina`/`olla_cocina`), cliente `client/src/cocina/panelCocina.ts` + `client/src/construccion/renderConstrucciones.ts` (`cocinaMasCercana`) + `client/src/game.ts`. Probado: `server/test/cocina.test.ts` (15 tests) + `server/test/platosCreadosBd.test.ts` (3 tests), suite completa de servidor 504/504, suite de interiores 34/34, `tsc --noEmit` limpio en `server/` y `client/`, `combate.e2e.mjs` en verde.
...
**Interpretación explícita del "hervir agua"**: la vasija es un construible autocontenido (como `colmena`/`trampa_pesca`) — no exige llenarla de agua ni encenderla por separado, "puesta al fuego" es el estado por defecto de la propia pieza. Simplificación deliberada frente al flujo literal descrito ("llenar de agua, poner al fuego, hervir"), documentada aquí en vez de construir una mecánica de encendido/agua separada sin que aporte jugabilidad real.
...
- "Hervir agua antes de añadir ingredientes" se simplificó a una vasija autocontenida sin paso de agua separado (§3) — si se quiere el paso literal, habría que añadir un estado "encendida"/"con agua" a la vasija.
