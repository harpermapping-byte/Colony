...
- **Persecución automática** ("se irá corriendo tu npc jugador a por el animal"): al recibir `caza:iniciada`, `cazaAutomaticaId` guarda la presa y el bucle de render, SOLO si no hay ninguna tecla de movimiento pulsada, manda `input` hacia la posición EN VIVO del animal (desde la posición real del servidor del jugador, no la interpolada), cuantizada a 16 rumbos — el servidor normaliza cualquier vector (`Math.hypot(dir.x, dir.y)`), y cuantizar respeta la regla "input solo al cambiar dirección" (sin ella se mandaría un paquete por frame). Nunca manda `correr`: andar ya supera a la especie más rápida (§4bis) y el sprint gasta estamina real. Se cancela sola con `caza:atrapado`, con `combate:error`, si la presa desaparece del Schema (murió/salió del interés) y **con cualquier WASD/flecha del jugador** — el control manual manda siempre.
...
**Verificación** (`playtestMultijugador.e2e.mjs`, paso 5 reescrito para el camino REAL: teletransporte a 7 casillas de la presa —fuera de `radioHuida`—, `page.mouse.click` sobre la proyección del animal, clic en el botón `Cazar`, espera de `caza:iniciada`, `__cazaAuto()` activo, espera de `caza:atrapado` sin tocar ninguna tecla, `__cazaAuto()` apagado al final): PENDIENTE_VERIFICACION_CAZA
