...
- **`GestorFaunaSalvaje.quitarIndividuo(id)`** (nuevo,
  `server/src/mundo/faunaSalvajeViva.ts`) — DISTINTO de `matarIndividuo`:
  marca la fila `estado: "domesticado"` (nuevo valor de
  `EstadoFaunaSalvaje`, `server/src/datos/bd.ts`) en vez de `"muerto"`, y
  **nunca crea cadáver** — un ciervo que se convierte en mascota no debe
  dejar un cuerpo looteable en su sitio. El único chequeo real que existía
  sobre `estado` (`faunaSalvajeSector.ts: vivo = estado==="vivo"`) ya trata
  cualquier valor que no sea `"vivo"` como no-vivo, así que el nuevo estado
...
- **Tests puros nuevos**: `comidaSirveParaDieta`
  ...
  `quitarIndividuo` (sin cadáver, estado `"domesticado"`,
  `server/test/faunaSalvajeViva.test.ts`), `ponerMonturaMascota`
...
- **Suite completa de servidor: 535/535.** `tsc --noEmit` limpio en
  `server/` y `client/`. Build de producción (`vite build`) verificado.
