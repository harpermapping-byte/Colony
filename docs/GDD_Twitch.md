...
**ESTADO: v1 IMPLEMENTADA Y VERIFICADA (2026-08-30) — solo la parte que no necesita autorización del streamer.** Piezas: `server/src/twitch/` (tipos.ts, titulos.ts, catalogoEventos.ts, registro.ts, gestorTwitch.ts, chatBot.ts, estadoDirecto.ts), `server/src/rooms/base/RoomExteriorBase.ts` (implementa `RoomConectable`: comandos de chat, eventos de mundo, título social), `server/src/rooms/schema/HubState.ts` (`Player.tituloTwitch`, `HubState.oscuridadAbsoluta`), `server/src/index.ts` (bootstrap del bot de chat + detección de directo, una vez por proceso). Probado: `server/test/titulosTwitch.test.ts`, `catalogoEventosTwitch.test.ts`, `registroTwitch.test.ts`, `gestorTwitch.test.ts` (24 tests nuevos, puros — sin BD real), suite completa de servidor 406/406, `tsc --noEmit` limpio en `server/`, y `client/test/twitch.e2e.mjs` contra un servidor Colyseus real (Modo Live forzado, `!comer`/`!beber`/`!curar`, canje "malo" aceptado, segundo canje "malo" rechazado por cooldown, canje "bueno" NO bloqueado por el cooldown de "malo" — 7/7 comprobaciones OK).
