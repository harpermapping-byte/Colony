...
**Bug real de cliente cerrado (2026-09-09, ver `docs/GDD_Cuentas.md` §5ter)**: el paso 5 de arriba (redirect con `?twitchSession=...`) hacía que la pantalla de login OBLIGATORIA de jugador (`docs/GDD_Cuentas.md` §5bis) se saltara entera si el jugador todavía no tenía cuenta — heredado de cuando esa cuenta era opcional. Cerrado sin tocar nada de este archivo (el servidor siempre funcionó bien): `debeSaltarBienvenida()` ya no trata `twitchSession`/`adminSession` como bypass por sí solos. Valores concretos de `CLIENT_URL`/`TWITCH_REDIRECT_URI` para el dominio real del streamer en `docs/GDD_Despliegue_Local.md`.
