...
**Panal de abejas SALVAJE, nuevo recolectable (2026-09-09, pedido streamer jugando: "las abejas... debería haber un panal de abejas del cual con click puedes sacar miel")**: hasta ahora la ÚNICA fuente de `miel` era la colmena construible (`interiores/catalogo/exteriores.json::colmena`, producción pasiva por intervalo, `docs/GDD_Produccion.md`) — no existía ningún hallazgo salvaje. Nueva entrada `panal_salvaje` en `baker/catalogo/vegetacion.json` (bioma `pradera`, el mismo que la fauna `abeja`, `categoriaRecurso:"miel"` — el MISMO itemId real que ya produce la colmena, dos fuentes del mismo recurso, no un duplicado) + entrada nueva en `CATEGORIA_HERRAMIENTA_RECOLECCION` (`miel: {oficio:"molinero", tier:3}` — molinero ya es el oficio de "comida/fibra silvestre" de esta misma tabla) — reusa el pipeline GENÉRICO de "coger" tal cual, cero código de servidor nuevo aparte de esa fila. Arquetipo visual propio `PANAL` en `taller-vox/generar_naturaleza.js` (no encaja en flor/hongo/arbusto — silueta de capas apiladas en lágrima invertida con agujero de entrada oscuro en la base, mismo criterio "una plantilla nueva si hace falta" de CLAUDE.md filosofía #7), 3 variantes exportadas a `assets/vegetacion/panal_salvaje_0{1..3}.glb`.

**"Colmenas fabricadas para que las abejas vivan ahí" — ya existía, gate a un "agricultor" NO añadido a propósito**: el resto de ese mismo pedido ("el agricultor a nivel alto puede hacer colmenas") ya está satisfecho por la `colmena` construible de siempre (`EntradaConstruible` con `produccion`, colocable con la tecla B por cualquier jugador con parcela) — verificado leyendo `server/src/construccion/catalogo.ts`/`produccion.ts`, sin código que escribir. Lo que NO existe es un oficio "agricultor"/"apicultor" jugable que la gatee — los 10 oficios reales (`OFICIOS_JUGADOR_VALIDOS`, `server/src/personaje/oficios.ts`) son herrero/carpintero/ingeniero/picapedrero/molinero/cazador/cocinero/curandero/curtidor/joyero, ninguno temáticamente de agricultura/apicultura; "apicultor" solo existe como profesión de NPC (`personajes/catalogo/npcs.json`), no de jugador. Añadir un 11º oficio jugable es un cambio de diseño grande (nueva mesa, herramientas, curva de nivel, economía) — NO se ha tocado sin el OK explícito del streamer (CLAUDE.md: "cambios grandes de diseño: proponer primero"), documentado aquí para que se decida aparte.

**Sin tocar, gap real documentado**: la fauna `abeja` (`GestorFaunaSalvaje`, `poblacionInfinita:true`) sigue vagabundeando alrededor de su punto de spawn del bake sin ninguna atracción hacia `panal_salvaje`/`colmena` cercanos — "ahora están rondando por rondar" sigue siendo cierto en el sentido de que no hay IA que las haga "vivir" en un panal concreto; conectar el wander de fauna viva a la posición de una estructura real es una pieza de IA aparte, más grande, no abordada en esta pasada. Verificado: `server/test/herramientasRecoleccion.test.ts` (test de cobertura "todo categoriaRecurso tiene tabla de gating") 17/17 con la fila nueva, servidor 1308/1308 sin regresión, bake real de `ejemplo-rapido.json` coloca `panal_salvaje` de verdad (14 instancias) sin errores nuevos, `.glb` de las 3 variantes validado estructuralmente.
...
## §0quater. RONDA 4 — NPCs de LORE (2026-08-31)

Pedido literal: "podemos añadir a los NPC estáticos que pone el admin unos 4 o 5 para lore... gente que cuando hables te cuente lore del servidor, que leerán o tendrán en un archivo... cuando termine el juego haré el lore y se pondrá ahí... misma regla nombres/generación etc.".

- **Mismo mecanismo exacto que los NPC tutoriales** (§0ter) — no un sistema aparte: `poblacion/catalogo/npcsTutoriales.json` gana un campo `categoria` (`"tutorial"` | `"lore"`, ausente = `"tutorial"` por compatibilidad) y 5 entradas nuevas (`lore_fundacion`, `lore_bandidos`, `lore_dioses`, `lore_guerra`, `lore_magia`) — mismo `admin:npcTutorial:colocar/catalogo/quitar`, mismo nombre real del listado de políticos (`lore_fundacion`→Felipe González, `lore_bandidos`→Pedro Sánchez, `lore_dioses`→Aznar, `lore_guerra`→Rajoy, `lore_magia`→Pablo Iglesias), mismo `titulo` de sabor aparte ("Guardián de la Memoria", "Cronista de las Guerras"...), mismo vestido vía `equipoVisual.ts`. `admin:npcTutorial:catalogo` ahora manda también `categoria`/`titulo` en cada entrada (antes solo id/nombre/mecanica) — para que un futuro panel pueda agrupar "tutoriales" y "narradores de lore" por separado.
- **El texto real vive en un archivo aparte, a rellenar más adelante**: `poblacion/catalogo/loreTexto.json` (`{ "textos": { "<id>": "..." } }`, vacío hoy a propósito) — "cuando termine el juego haré el lore y se pondrá ahí", pedido literal. `cargarLoreTexto()` (`server/src/mundo/npcsFijos.ts`) lo lee EN CALIENTE, sin caché, en cada `npc:hablar` — a diferencia del catálogo de arquetipos (que sí se cachea, cambia poco), este archivo se espera que se rellene en caliente sin reiniciar el servidor. Un NPC de lore sin su clave en `loreTexto.json` todavía cae al mismo placeholder que un tutorial sin texto (`[Lore pendiente de escribir: <tema>]`), solo que dice "Lore" en vez de "Tutorial".
- Verificado: `server/test/oficios.test.ts` (2 tests nuevos: catálogo con ≥4 NPC de lore y título real cada uno; `cargarLoreTexto` lee en caliente — rellenar una clave a mano se nota sin reiniciar el proceso, sin caché de por medio), `server/test/oficios.e2e.mjs` confirma 17 arquetipos en el catálogo real (antes 12), `tsc` limpio, servidor 863/863.

## §0quinquies — RONDA 5: bug real que hacía inalcanzable "oficio:elegir" + UI de cliente real (2026-09-10)

Pedido streamer, dos preguntas seguidas: *"como cojo las profesiones nada mas entrar? el maestro de oficios lo tengo que poner yo como jarl o sale ya bakeado?"* y *"habra que hacer la ui de la conver con el para elegir no? click sobre el elegir oficio?"*.

**Bug real encontrado investigando la primera pregunta, cerrado el mismo día**: el "Maestro de Oficios" (`tutorial_oficios`, §0ter arriba) SÍ es plantable por el jarl con `admin:npcTutorial:colocar` — pero `npcTutorialAAgente` (`server/src/mundo/npcsFijos.ts`) le daba SIEMPRE el mismo marcador genérico `oficio: "npc_tutorial"` a CUALQUIER tipo de NPC tutorial, sin distinguir cuál. `npcMaestroOficiosMasCercano` (`RoomExteriorBase.ts`, el gate real de `oficio:elegir`/`oficio:cambiar`) solo reconoce el valor literal `"maestro_oficios"` — así que el NPC "Maestro de Oficios" plantado por el jarl era funcionalmente invisible para ese gate: cualquier jugador que hablara con él y mandara `oficio:elegir` recibía siempre `"necesitas hablar con el maestro de oficios"`, aunque lo tuviera delante. Ningún jugador podía elegir oficio, con o sin el NPC plantado. Arreglado en UNA línea (`npcTutorialAAgente`): `oficio: fila.tipoTutorial === "tutorial_oficios" ? "maestro_oficios" : "npc_tutorial"` — el resto de tipos (construcción, crafteo, reclutador...) siguen con el marcador genérico de siempre, sin cambio de comportamiento. **El NPC NO viene bakeado en ningún mapa** — sigue siendo responsabilidad del jarl plantarlo a mano (mismo mecanismo de §0ter, `admin:npcTutorial:colocar {tipoTutorial:"tutorial_oficios"}` desde dentro del juego, en su posición actual).

**UI de cliente real, cerraba el otro hueco documentado desde 2026-08-30 ("sin panel de cliente todavía")**: `client/src/npc/panelDialogoNpc.ts` (el mismo panel de conversación con NPCs con IA, tecla H/icono del dock 💬) gana un bloque de selector de oficios que aparece SOLO cuando el NPC con el que se habla es el maestro de oficios (`Npc.tipoTutorial === "tutorial_oficios"`, ya replicado al cliente) — 10 botones, uno por oficio; con un slot vacío, clicar uno lo elige gratis (`oficio:elegir`); con los 2 slots llenos, clicar un oficio nuevo pide confirmar cuál de los 2 actuales reemplaza (`oficio:cambiar`, cuesta Farycoins reales — el precio no se calcula en cliente, lo dice el servidor en el toast de éxito/error). Toasts nuevos (`registroCombate.ts`, mismo overlay ya usado para combate/personaje/higiene) para `oficio:elegido`/`oficio:cambiado`/`oficio:error` — antes `oficio:error` solo se logueaba en consola y `oficio:elegido`/`oficio:cambiado` no tenían NINGÚN feedback visible jugando.

**Bug real de carrera de red encontrado con el propio e2e, antes de dar la UI por buena**: la primera versión refrescaba el selector releyendo `room.state.players.get(sessionId).oficio1/oficio2` justo al recibir `oficio:elegido` — pero ese mensaje llega por su PROPIO canal (inmediato), más rápido que el siguiente patch de Schema (15hz), así que el toast salía bien pero el botón nunca pasaba a "✓" (el `room.state` todavía tenía el valor viejo en ese instante exacto). Arreglado sin depender de ningún timing de red: el panel mantiene una copia LOCAL de oficio1/oficio2 mientras el selector está abierto, y la actualiza directamente con el `slot`/`oficio` que el PROPIO mensaje del servidor ya lleva (`oficio:elegido`/`oficio:cambiado` incluyen `slot` desde que existen) — nunca vuelve a leer `room.state` para esto.

Verificado: `server/test/oficios.test.ts` (+1: `npcTutorialAAgente` da `maestro_oficios` SOLO a `tutorial_oficios`, el resto del catálogo sigue con `npc_tutorial`), servidor 1311/1311, `tsc --noEmit` limpio en cliente y servidor, cliente 55/55. **E2E nuevo real de punta a punta** (`client/test/oficioSelectorNpc.e2e.mjs`, servidor+cliente+Playwright reales): login real, el jarl planta al maestro de oficios en su propia posición, abre el diálogo con H, elige 2 oficios gratis (el botón pasa a "✓" al instante, sin cerrar el panel), un 3er clic con los 2 slots llenos pide confirmar reemplazo, y el reemplazo real (con Farycoins dados por `admin:debug:ajustarFarycoins`) se completa de verdad — 11/11 comprobaciones en verde, con capturas.
