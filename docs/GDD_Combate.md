...
- **No verificado con un E2E de punta a punta con fauna acuática real viva**: seedear un individuo de `fauna_salvaje` (BD) que se active de verdad en un sector (`GestorFaunaSalvaje.activarSector`) para disparar un combate real contra una orca requeriría entender a fondo el ciclo hambre/sed/activación de sector, fuera del alcance razonable de esta pasada — cubierto en su lugar por tests puros (`elegirArena` filtra por terreno, `requiereAgua` viaja tal cual del catálogo) más `tsc --noEmit` limpio en toda la cadena roster→Schema→cliente.
