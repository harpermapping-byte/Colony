...
**ESTADO: ✅ CONFIRMADO por el streamer (2026-08-30).** Decisión explícita: este sistema (combate táctico por turnos sobre rejilla isométrica, arena 8x8/10x10, AP/MP, UI de combate) **SUSTITUYE** al sistema de daño directo simple que se había implementado mientras tanto (`server/src/combate/combate.ts` + mensaje `combate:atacar` de `HubRoom.ts`, docs/GDD_Mecanicas.md §5.4) — ese sistema queda como INTERINO, funcionando hasta que el camino interactivo de ESTE documento esté cableado, momento en el que se retira. Excepción explícita del streamer: **cuando ninguno de los combatientes es un jugador** (NPC contra animal, NPC contra NPC) el combate NO se juega por turnos interactivos con UI — se AUTOSIMULA server-side, de golpe, con el mismo motor de reglas (ver §7). Responde al pedido explícito: un sistema de combate táctico por turnos sobre rejilla isométrica, server-autoritativo en Colyseus a coste cero y renderizado en el cliente con la cámara isométrica que ya existe.
...
- **Todavía sin disparador real**: igual que `matarIndividuo`/la caza de depredadores, esto es el mecanismo — nadie detecta hoy un encuentro NPC-vs-animal o NPC-vs-NPC para llamarlo (no existe guarnición viva en red, ni agro de fauna conectado a NPCs). Se conecta cuando exista ese disparador.
