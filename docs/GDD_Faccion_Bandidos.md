...
- **Pendiente dentro de fase 1, no bloqueante**: sembrar automáticamente una fila de `asentamientos` por cada POI `asentamiento_hostil` ya bakeado (hoy `obtenerOCrearAsentamiento` crea la fila perezosamente la primera vez que se referencia, pero nada la referencia todavía — falta el punto de enganche real, candidato natural: cuando `RegionRoom` carga una región cuyo `indice.json` es de tier `asentamiento_hostil`) y un tick periódico real (`clock.setInterval`, mismo criterio que `GestorAgentes`) que llame a `ejecutarTickEconomia`.
