...
- **Pendiente dentro de fase 1 — RESUELTO (2026-08-28)**: `RegionRoom.onCreate` (`server/src/rooms/RegionRoom.ts`) lee `indice.tier` del bake; si es `asentamiento_hostil`, siembra la fila (`obtenerOCrearAsentamiento(mapaId)`, idempotente) y arranca `this.clock.setInterval(() => ejecutarTickEconomia(bd), MS_POR_TICK_ECONOMIA)` — mismo criterio "perezoso" que `GestorAgentes`/`GestorFauna`: el tick SOLO corre mientras esa room concreta tenga jugadores (auto-dispose al vaciarse cierra también la conexión a BD en `onDispose`), nadie paga por simular una aldea bandida que nadie visita. Cadencia: un pulso por HORA de juego (`assets/mundo/tiempo.json`: 30 min reales/día → 75s reales/hora), no un número de balance inventado aparte — mismo ritmo que ya rige turnos de guardia/rutinas de NPC. Override `TICK_ECONOMIA_MS` solo para tests (mismo patrón que `HORA_FORZADA`/`BD_RUTA`).
  - Verificado con un E2E real y repetible (`server/test/faccionBandidos.e2e.mjs`, `node server/test/faccionBandidos.e2e.mjs`): banquea un `asentamiento_hostil` de prueba, arranca el servidor con BD temporal y tick acelerado (2s), se une por Colyseus SIN navegador, confirma desde FUERA de la room (leyendo la misma sqlite) que (a) la fila se sembró al entrar y (b) tras insertar una tropa de prueba, dos pulsos reales del `setInterval` escriben producción real en SQLite (madera/piedra/hierro). Limpia sus propios artefactos (mapa de prueba + sqlite temporal) al terminar.
  - **Sigue sin resolver, y sin fecha**: sembrar TROPAS reales al crear el asentamiento (cuántas, de qué `rango`, según qué criterio — `riquezaLoot`/`temasEnemigo` del tipo de dungeon son candidatos, pero no hay decisión tomada) — sin tropas sembradas, el tick corre pero no produce/consume nada (0 población viva). No bloquea nada de lo ya hecho; es la costura natural con "Patrullas en vivo" de la fase siguiente.
