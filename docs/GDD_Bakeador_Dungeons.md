...
### 3bis. Profundidad BFS + bucles (pedido 2026-08-31)

Evaluado (y reaprovechada la idea, no el código) un generador ajeno de dungeons en Three.js con el mismo algoritmo base que ya teníamos (room-scatter + MST de Prim's) — coincidencia real, no copiado; investigamos e implementamos esto por nuestra cuenta (§0). Dos piezas suyas sí faltaban aquí y se añadieron:

- **Tamaño de sala según profundidad en el grafo, no al azar puro**: antes cada sala (rectangular u orgánica) salía con un tamaño uniforme al azar dentro de un rango fijo, sin relación con su posición en la mazmorra. Ahora `generarPlantaMazmorra` calcula primero una **topología aproximada barata**: coloca cuadrados de tamaño medio compartido (mismo lienzo, mismo rechazo por solape) SOLO para tener centros con los que construir un MST y correr BFS desde la sala 0 (tratada como "entrada") — `bfsProfundidades(n, aristas)`, nuevo, puro. Con la profundidad de cada sala ya conocida (0 = pegada a la entrada, 1 = el punto más profundo del grafo estimado), el tamaño real se decide DENTRO de ese sesgo: orgánicas van de 9-18 (cerca) a 13-28 (al fondo); rectangulares se sesgan hacia el máximo de su propio rango declarado en `tipos_sala.json` (nunca se salen de lo que ese tipo de sala permite). La colocación REAL después puede acabar en posiciones algo distintas a la aproximación — asumido a propósito, es solo una estimación para dimensionar, no la mazmorra final.
- **Sala de jefe/conector por profundidad, no por área**: antes era simplemente "la sala más grande". Ahora, tras la colocación REAL, se recalcula el MST y el BFS otra vez (sobre posiciones reales, no la aproximación) y la sala de jefe es la de **mayor profundidad real** en el grafo final (empate roto por área) — coherente con "el jefe está al fondo de la mazmorra", no "el jefe está en la sala que salió más grande por azar".
- **Bucles sobre el MST**: un MST conecta todo con el mínimo de corredores — siempre hay un ÚNICO camino entre dos salas cualesquiera, nunca se puede rodear. `elegirAristasBucle(salas, aristasMST, rnd)`, nuevo y puro: reinserta una fracción (35% de probabilidad, tope ~1 bucle cada 4 salas) de las aristas "casi tan cortas" como la que cada sala ya usa en su propio MST — nunca una arista lejana o rara, y nunca una que YA esté conectada — dando rutas alternativas reales sin disparar el número de corredores.
- **Verificado**: 8 tests nuevos en `mazmorras/test/mazmorra.test.js` (BFS con cadena/rutas múltiples/grafo vacío, bucles nunca duplican una arista del MST, respetan el tope, respetan la probabilidad inyectada, y una prueba estadística sobre 5 semillas de `cueva_goblins` confirmando que las salas profundas salen más grandes en media que las cercanas a la entrada). Suite completa sin romperse: `mazmorras/test/mazmorra.test.js` 13/13, `server/test/mazmorra.test.ts` 3/3 (la conectividad real por flood-fill sigue intacta — los bucles solo AÑADEN corredores, nunca quitan los del MST), suite completa de servidor 917/917.
...
- **Cooldown de 1h**: `server/src/datos/bd.ts` gana `obtenerLimpiezaMazmorra`/`marcarMazmorraLimpiada` + tabla `mazmorras_estado` (SQLite dev / Postgres prod, mismo patrón dual que construcción). **Conectado (2026-09-01)**: `DungeonRoom.finalizarMuerte` llama a `marcarMazmorraLimpiada` cuando muere el último `Enemigo` vivo de `state.enemigos` — detalle del trigger y su verificación en `docs/GDD_Combate.md` §3/§9.2 (una auditoría de este mismo GDD encontró que llevaba desconectado desde que se escribió esta línea, sin ningún call site real).
...
- ~~**Trigger de "mazmorra limpiada"**: la persistencia del cooldown (`mazmorras_estado`) está lista pero nada llama a `marcarMazmorraLimpiada` todavía~~ — **conectado 2026-09-01**, ver §4.3 y `docs/GDD_Combate.md` §3/§9.2. ~~Sigue pendiente, real y aparte, el hueco que destapó verificarlo con combate de verdad: una `InteriorRoom`/`DungeonRoom` usa el `autoDispose` por defecto de Colyseus, así que un jugador que sale SOLO a pelear el último enemigo a la arena puede dejar su room de origen vacía (y por tanto autodestruida) antes de que el combate resuelva — el resultado (esta limpieza, loot, XP) nunca llegaría a aplicarse. Afecta a cualquier combate en solitario contra el último ocupante de un interior, no solo a este trigger.~~ **Arreglado también, misma fecha** (`docs/GDD_Combate.md` §9.2: `autoDispose` dinámico en `RoomExteriorBase`, atado a `state.combatesEnCurso` — verificado con `client/test/combateSoloMantieneRoom.e2e.cjs`, un jugador solo contra el último enemigo de una mazmorra).
