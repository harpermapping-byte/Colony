...
**CORREGIDO (2026-09-13), producción nunca llegó a parecerse a lo aprobado — bug real de mismatch demo-vs-producción, no una preferencia**: pedido streamer, "el suelo no se ve ni parecido a como vimos en las imagenes de prueba, se ve repetitivo y con poca resolución, haría falta más cuadraditos o pixeles pequeños y variaciones". Confirmado leyendo el código en vez de solo recalibrar a ojo: `client/test/texturaSueloComparacion.ts` (la comparación A/B/C de arriba, la que el streamer vio y aprobó con "la prueba B es lo que hay que hacer") usa `PX_POR_TILE_B=10` — pero `sectorVisual.ts::PX_POR_TILE_SUELO` se dejó en `2` al pasar a producción, 5 veces menos resolución que lo aprobado. A `PX=2` el guard `tam>=4` de más arriba (guijarros/mampostería) NUNCA se cruzaba, así que la producción real solo mostraba el moteado base — la parte "con detalle" del diseño aprobado nunca llegó a activarse fuera de las capturas de prueba, de ahí que se viera "repetitivo y con poca resolución" tal cual describe el streamer. **Arreglado subiendo `PX_POR_TILE_SUELO` de 2 a 10** (el mismo valor exacto de la demo aprobada, para eliminar cualquier ambigüedad) — cruza el guard de detalle de área de sobra. **Segundo hallazgo, ligado al "variaciones" del mismo mensaje**: `NUM_VARIANTES_PATRON=4` significa que dos casillas VECINAS de la misma familia/color tienen 1 entre 4 de mostrar el parche EXACTO (cada variante es un parche completo precalculado, no ruido independiente por píxel) — percibible como repetición aparte de la resolución. Subido a 8, a coste real CERO en `crearTerrenoSector` (las variantes de un id se cachean UNA vez para todo el mapa/sesión, nunca por sector ni por casilla). Rebenchmarkeado con el mismo arnés que fijó el valor original, mismo sector más pesado del mapa principal (`sector_009_001.json`, 320x320): PX=1 ~130-139ms (baseline), PX=2 (valor viejo) ~140-151ms (+10ms), PX=4 ~163-172ms (+35ms), PX=8 ~245-261ms (+115ms), **PX=10 (valor nuevo) ~266-296ms (+145ms)** — sigue siendo un coste ONE-TIME por sector materializado (no por frame), y este sandbox sigue sin GPU real (`docs/GDD_Rendimiento.md` §7) así que los absolutos no representan hardware real, solo la comparación relativa. Verificado con capturas reales antes/después del `CanvasTexture` en bruto (`client/test/capturas/patron_suelo_raw_px{2,8,10}.png`, gitignoradas, no comiteadas — mismo criterio que el resto de capturas "Aisladas" del proyecto): a PX=2 el césped/tierra se ve un moteado grueso y uniforme; a PX=10 se distinguen briznas/flores/guijarros individuales, coincidiendo con lo que mostraba `texturaSueloComparacion.ts` en su día. `cd client && npx tsc --noEmit` limpio, `cd client && npx tsx --test test/*.test.ts` sin regresión. **Sin verificar en vivo con el streamer en su propio hardware** — mismo criterio honesto del resto de esta sesión, aunque esta vez la resolución elegida es la MISMA que ya vio y aprobó, no una nueva a ciegas.

## Patrón horneado extendido a agua/hielo/lecho/nieve acumulada + textura real en murallas/muros (2026-09-13, pedido streamer: "aplicalo tambien a lo que falta si, y tambien habria que ver como hacer algo asi sobre las murallas y muroos de ciudades aldeas que se ven planos")

El bullet anterior (patrón de suelo) dejó agua/hielo/lecho SIN patrón A PROPÓSITO ("su propio tratamiento translúcido/lecho es más complejo que un parche RGBA opaco") — el streamer, tras confirmar qué quedaba fuera, pidió cerrarlo. `patronTerreno.ts` gana 4 familias nuevas (`FamiliaPatronTerreno` pasa de 6 a 10): **"agua"** (ondas — trazos horizontales cortos y sutiles, más claros que la base), **"hielo"** (grietas finas — paseos aleatorios cortos), **"lecho"** (grano fino disperso, MISMO algoritmo que "arena" — un sedimento de río/mar es tan fino como una duna, solo cambia el color base que ya llega calculado por casilla) y **"madera"** (tablas verticales alternas + junta oscura + un par de vetas horizontales, para los muros de empalizada). Las 4 generan siempre RGBA OPACO (alfa=255, igual que las 6 originales) — nunca mezclan alfa dentro del propio patrón.

**El agua necesita alfa translúcido, no opaco — nueva función `obtenerParchesTerrenoTranslucido(familia, color, tam, alfaByte)`**: genera el patrón normal (opaco) y le fuerza el canal alfa a un valor uniforme DESPUÉS, en una copia aparte (cacheada por `(familia,color,tam,alfa)`, nunca muta la caché opaca compartida) — mezclar RGB y alfa en el mismo paso habría abierto agujeros de transparencia irregulares (un patrón de "rizos" con huecos, no ondas de color) en vez de una superficie translúcida uniforme con solo el COLOR variando. `sectorVisual.ts` la usa para agua (`agua`/`agua_profunda`, alfa real del catálogo — 0.45/0.55) reusando el MISMO canvas/resolución de suelo (`PX_POR_TILE_SUELO`, sin canvas nuevo); hielo usa `obtenerParchesTerreno("hielo", ...)` directo (siempre opaco, no necesita la variante translúcida) — ninguno de los dos pasa por `familiaPatronTerreno()` (que los excluye a propósito para los ids REALES de catálogo), la familia se pide EXPLÍCITA en el punto de uso.

**Lecho (fondo de mar/río) y nieve acumulada estacional necesitaban su PROPIA resolución** — a diferencia de agua/hielo/suelo (que ya vivían en el canvas de suelo a `PX_POR_TILE_SUELO`), estos dos eran canvas APARTE (`fondo`/`nieveCanvas`) a 1px/casilla desde el diseño original. Subirlos al mismo PX=10 del suelo habría pagado ese coste en el sector ENTERO (ambos cubren el rectángulo completo, no solo donde hay agua/nieve reales — un canvas no puede recortarse por forma) — se les dio una resolución MENOR y propia, `PX_POR_TILE_FONDO`/`PX_POR_TILE_NIEVE = 4`: el lecho se ve casi siempre A TRAVÉS del agua translúcida (blend con la superficie, poco beneficio de más detalle) y la nieve acumulada es casi blanco uniforme — 4 basta para que ninguno de los dos se vea ya un color plano sin duplicar el coste ya asumido para el suelo. El margen de arenas (`extenderConMargenClamp`, usado solo en combate) se corrigió para escalar por la resolución PROPIA de cada canvas (antes asumía 1px/casilla para fondo/nieve a secas).

**Bug real encontrado calibrando la nieve acumulada, antes de darlo por bueno con captura**: `parchesNieveTierra` se generaba con base `[255,255,255]` (blanco puro) — pero los "destellos" de la familia "nieve" siempre mezclan HACIA 255 (`generarParcheTerreno`), y `r + (255-r)*factor` con `r=255` da `255` sin cambio ninguno: con blanco puro de base, los destellos salían matemáticamente invisibles. Corregido a `[250,250,252]` (imperceptible como tono plano, pero deja margen real para que los destellos se noten) — el moteado BASE (común a las 10 familias) ya aportaba algo de variación incluso con blanco puro (mezcla también hacia 0 para la mitad de los píxeles), así que el bug no dejaba la capa totalmente lisa, pero sí más plana de lo que el diseño de "nieve" prometía.

**Murallas/muros de ciudades y aldeas ("se ven planos")**: confirmado que hay DOS sistemas de render para un muro real (`ciudades/src/generar.js` pinta `muralla_piedra`/`empalizada` como TERRENO, terreno real que consume el juego) — (1) `ALTURA_TERRENO_SOLIDO` en `crearTerrenoSector`: una `InstancedMesh` de cajas 1x1xaltura por CASILLA de muro recto, antes con un único `MeshStandardMaterial({color})` plano por tipo; (2) `crearMurallaSector`: torres (2.2x·x2.2) y postes de puerta (1.8x·x1.8 piedra / 0.35x·x0.35 madera) como `THREE.Mesh` sueltos, mismo color plano. Ambos sin ningún detalle — exactamente "planos". Cerrado con `texturaMuro(id, repeatU, repeatV)`+`materialesMuro(id, ancho, altura)` (nuevos, `sectorVisual.ts`): un canvas `PX_POR_UNIDAD_MURO=12`x12 con el patrón horneado de la familia correspondiente (`familiaMuro`: "madera" para empalizada, "roca" para muralla_piedra/roca_inaccesible — reusa `familiaPatronTerreno` cuando aplica, con el caso especial de empalizada que esa función mapea a "tierra" por ser el catch-all de su textura de SUELO, invisible bajo la caja) aplicado como `map` con `RepeatWrapping` — las 4 caras laterales tilean el mismo parche `repeatU` veces en horizontal y `repeatV` (=altura real en unidades) en vertical, dando "hiladas" apiladas sin estirar nada; arriba/abajo reusan el mismo patrón con `repeat=(ancho,ancho)` (huella real, sin la repetición vertical de las caras laterales). Todos los usos reales de este archivo tienen huella CUADRADA (1x1 los muros rectos; 2.2x2.2/1.8x1.8/0.35x0.35 las torres/postes) — un único `repeat` por caja sirve para las 4 caras sin deformar ninguna, no hizo falta manejar aspecto rectangular. **Variación real por instancia en los muros rectos** (`InstancedMesh.setColorAt`, soportado NATIVAMENTE por `MeshStandardMaterial` sin necesitar `vertexColors:true` — confirmado leyendo el propio `three.module.js`: `instancingColor` se activa solo mirando `object.isInstancedMesh && object.instanceColor !== null`, nada más): un tono `0.92..1.08` determinista por casilla del mundo (`hashCasilla`) rompe la monotonía de que TODA la muralla comparta literalmente el mismo textura-tile de 12x12 sin ninguna variación entre segmentos.

Verificado: `client/test/patronTerreno.test.ts` +7 tests (las 4 familias nuevas sin excepción y siempre opacas, "lecho" produce BYTE A BYTE lo mismo que "arena" con el mismo color/semilla, "madera" tiene ≥3 tonos distintos — no un color plano disfrazado, `obtenerParchesTerrenoTranslucido` conserva RGB y fuerza alfa exacto, cachea por `(familia,color,tam,alfa)` con referencias distintas por alfa, y NUNCA muta la caché opaca compartida), `cd client && npx tsc --noEmit` limpio (necesitó copiar el `Uint8ClampedArray` devuelto por `generarParcheTerreno` a uno CONSTRUIDO localmente antes de pasarlo a `new ImageData(...)` — la lib DOM actual exige `Uint8ClampedArray<ArrayBuffer>` y el valor de retorno de una función de otro módulo se infiere como el `ArrayBufferLike` más ancho, mismo patrón ya usado para `datosSuelo`), cliente 138/138 sin regresión. Rebenchmarkeado con el mismo arnés que fijó `PX_POR_TILE_SUELO` (`client/test/patronSueloAisladoCaptura.mjs`, sector más pesado del mapa principal): ~340-480ms por sector (varias ejecuciones, ruido real del sandbox sin GPU) frente a ~266-296ms de la pasada anterior — +80 a +180ms por las 4 texturas nuevas combinadas, sigue siendo un coste ONE-TIME por sector materializado, nunca por frame. Verificado VISUALMENTE con capturas reales: `client/test/capturas/patron_suelo_{cercano,zoom_juego_real}.png` (agua de río con ondas visibles en vez de un negro/azul plano), `client/test/capturas/nieve_aislado_borde_nivel_4.jpg` (capa de nieve con moteado real, ya no un blanco perfectamente liso), y un script de captura AISLADO desechable (mismo patrón "Aislado" del resto del proyecto, NO comiteado) contra un sector real de `ciudad_demo` con tramos de `muralla_piedra` — confirma mampostería con hiladas apiladas claramente visibles en el muro, antes un rectángulo de color liso. El recurrente "FALLO...404" del arnés de captura sigue siendo el mismo hallazgo preexistente y ajeno ya documentado en el bullet anterior (aparecía IDÉNTICO antes de tocar nada en esta pasada). **Pendiente real**: sin verificar en vivo con el streamer en su propio hardware; la textura de muro es la MISMA en TODOS los tramos de un tipo (solo varía el tono por instancia, no el dibujo — un mosaico de varias texturas por tramo sería la mejora natural si hace falta más variedad real); las torres/postes de `crearMurallaSector` no llevan la variación de tono por instancia (son `THREE.Mesh` sueltos, no instanciados — de menor cantidad, se dejó sin ese pulido menor por alcance).
