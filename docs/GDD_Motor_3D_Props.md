...
- **Carga perezosa de sectores + luz que sigue a la cámara**: para el mapa principal grande — el mapa demo cabe entero en memoria y en la cámara de sombra actual; uno de 100x100+ no. **Ya no es hipotético**: el mapa principal existe en `assets/mapas/principal/` (bakeado por el streamer, 100x100 chunks = 3200x3200 casillas, 100 sectores de ~1MB, 734.596 props, validación del bakeador en verde y todos los ids/terrenos verificados contra los catálogos reales). El cliente sigue apuntando a `demo` (`RUTA_MAPA` en `game.ts`) a propósito: cargar los 70MB de golpe con el cargador actual es inviable — implementar la carga perezosa por cercanía (cada sector ya es un fetch independiente en `cargarMapa.ts`) ANTES de cambiar `RUTA_MAPA`.
