...
- **Elevación en niveles discretos** (7 bandas: 0-6, agua profunda→cumbre inaccesible). Un salto de más de un nivel entre casillas vecinas INMEDIATAS genera automáticamente `roca_inaccesible` (comportamiento de siempre, sin tocar). **Medido 2026-08-29**: con el ruido de elevación actual (pensado para colinas suaves y un mundo caminable, no un laberinto de muros) ese salto de ≥2 bandas entre vecinos INMEDIATOS es rarísimo o inexistente en la práctica — en un mapa de prueba de 384×384 casillas, el delta máximo entre vecinos horizontales nunca pasó de 1. Retocar el ruido de elevación para que sea más abrupto NO se hizo — comparte campo con la hidrología (ríos) y la clasificación de bioma (altitud enfría temperatura), así que "más desnivel" ahí movería ríos y fronteras de bioma sin querer. **Se probó también sumar una capa de ruido de alta frecuencia SOLO a la banda discreta (no a la elevación continua) para forzar saltos reales de 2-3 niveles ahí mismo — funcionaba (comprobado: saltos de hasta 3 niveles, roca_inaccesible real en el corte) pero se descartó en favor de la solución de abajo** ("Rocas de acantilado", detección por PENDIENTE): forzar la banda vuelve intransitable de golpe una franja de montaña más ancha de lo esperado (más `roca_inaccesible` = menos terreno pisable, sin validar a fondo el efecto sobre el trazado de caminos a escala de producción), mientras que la decoración por pendiente da el mismo resultado visual (rocas grandes marcando el desnivel) sin tocar transitabilidad ni terreno.
- **Variantes de terreno dentro del mismo bioma — dos capas independientes**: (a) "variante" (ya existía): alternativas MECÁNICAS del terreno base — costa: playa arenosa o `playa_rocosa` (distinto `modVelocidad`); pradera/bosque: césped o `cesped_ralo` (transición real hacia tierra desnuda, en manchas). (b) "subvariante" (nueva, pedido 2026-08-29): capa de ruido INDEPENDIENTE que elige entre 2-3 subbiomas puramente COSMÉTICOS del terreno base final (mismo `transitable`/`modVelocidad`/`estratigrafia`, solo cambia `colorDebug`/textura futura) — cesped/nieve/arena tienen 3 looks, barro/ceniza/playa/roca tienen 2 — para que el suelo bakeado no sea un único tile repetido hasta la saciedad. Ver `baker/src/terreno.js` (`SUBVARIANTES`). `tierra_baldia` y `suelo_barbecho` ya están en el catálogo pero reservados para la futura mecánica de fertilidad del suelo (igual que `tierra_labrada` para cultivos) — el bakeador no los coloca todavía en el mapa salvaje.
- **LÍMITE DURO de 36 tipos de terreno**: `baker/src/exportar.js` codifica el terreno de cada casilla como UN SOLO CARÁCTER base-36 (0-9, a-z) — el catálogo (`terrenos.json`) no puede superar 36 entradas reales (sin contar claves `_nota*`, ya filtradas) sin corromper en silencio el terreno de los chunks que toquen un índice ≥36. Hoy está EXACTO en 36, cero hueco libre — documentado en `terrenos.json` (`_nota_limite_36`) para que la próxima ampliación no repita el bug (encontrado y corregido en esta misma pasada: se añadieron 13 subbiomas de golpe, 3 tuvieron que recortarse). Antes de añadir un terreno nuevo: hay que arreglar la codificación primero (relleno a ancho fijo o similar, tocando también el decodificador del cliente) o retirar alguno existente.
- **Rocas de acantilado** (pedido 2026-08-29, "que se vea el desnivel"): dado que el salto inmediato ≥2 bandas casi nunca dispara (punto de arriba), la decoración de acantilado se decide por PENDIENTE en un radio de 5 casillas (cruz N/S/E/O) — si la banda cambia ≥2 dentro de ese radio, es una ladera real y se plantan rocas grandes (`roca_acantilado_grande`/`_pequena`, `baker/catalogo/rocas.json`) de forma DETERMINISTA (no por el decorador de densidad normal), con una probabilidad (subida de 0.45 a 0.65 el mismo día, pedido del streamer: "que se vean más a menudo") por casilla de borde para no amontonar miles de piezas en una ladera larga. Terreno de suelo sin tocar — es decoración encima, no redefine `idTerreno`. **Placeholder a propósito** (mismo criterio que el resto del arte del proyecto): la forma/detalle real se bakeará más adelante con el arquetipo "roca" ya existente de `taller-vox/generar_naturaleza.js`; hoy solo fija el hueco de catálogo y la escala (grande ×2.6, pequeña ×1.3). Verificado: 1551-2107 rocas según semilla, ninguna en la cumbre en sí (banda 6 pura, ya `roca_inaccesible` sin necesitar refuerzo).
- **`roca_inaccesible` con volumen 3D de verdad** (`client/src/render3d/sectorVisual.ts`, `ALTURA_TERRENO_SOLIDO`, pedido 2026-08-29): hasta entonces el suelo exterior se renderizaba SIEMPRE plano — la elevación bakeada solo cambiaba de color, ninguna casilla de terreno exterior tenía altura de malla (solo las murallas de ciudad la tenían). `roca_inaccesible` entra en esa misma tabla (mismo mecanismo, sin código nuevo) — ahora un acantilado/cumbre real se ve como un muro que sobresale, no una mancha gris en el suelo.
- **Subir a lo alto — el camino en zigzag YA EXISTE, no hacía falta nada nuevo**: se investigó si hacía falta un mecanismo de "senda de escalada" aparte y NO — `baker/catalogo/pois.json` (`montana_nevada`) ya tiene 3 plantillas con `reglasSitio: ["bandaElevacionMin:4"]` (`cabana_cazador`, `mina_abandonada`, `tienda_cazador`), y en cuanto `colocarPOIs` sitúa una de ellas, el buscador de caminos (`caminos.js`) ya las conecta con la carretera principal usando el mismo zigzag por desnivel del punto de arriba — banda 5 es transitable (solo más costosa, `rugosidad×4`), banda 6 es la única realmente infranqueable (`Infinity`). Verificado con 4 bakes de prueba (bordes forzados a montaña): validación OK y 0 POIs sin ruta en todos, incluida una `mina_abandonada` real con su camino de acceso.
- **Vegetación fuera de la banda de montaña/cumbre** (pedido 2026-08-29, "arriba no salen plantas ni árboles ni arbustos"): `baker/catalogo/vegetacion.json` tenía 112 especies SIN `bandaElevacionMax` — el filtro por bioma (`entradasValidas`, `decoracion.js`) no bastaba, porque `bosque` clasifica hasta banda 5 (su propio rango `bandaElevacion` en `biomas.json`), así que un roble podía aparecer en una franja de bosque que resultara estar a banda 5 (montaña) por pura coincidencia del ruido. Añadido `bandaElevacionMax` a las 112: **4** para la vegetación normal (nunca en banda 4+, colinas para arriba las deja el bosque/pradera de verdad), **5** para las 8 especies alpinas ya restringidas a `montana_nevada` (nunca en la cumbre desnuda, banda 6 — coherente con que la decoración ya se salta cualquier casilla `!transitable`, así que en `roca_inaccesible` de verdad nunca hubo vegetación, esto cierra el hueco de las laderas walkable de banda 5 con biomas que no deberían tener flora de bosque/pradera).
- **Minerales concentrados de verdad arriba** (mismo pedido): las vetas de metal precioso EXCLUSIVAS de `montana_nevada` (`veta_hierro`/`veta_plata`/`veta_oro`/`veta_platino`) ganan `bandaElevacionMin: 5` (antes podían salir en cualquier punto de `montana_nevada`, banda 5 o 6 — ahora solo en la mitad alta transitable, justo antes de la cumbre) y `densidadBase` subida ~75% para que se note. `veta_cobre`/`veta_marmol` NO llevan el mismo límite de banda (aparecen también en `desierto`/`pradera` respectivamente, forzar banda 5 las habría eliminado de ahí) — solo densidad subida, más moderada.
- **Orillas de barro con zonas de arcilla, y escombros al pie de acantilado** (pedido 2026-08-29, "no toda la orilla, solo algunas zonas, aleatorio" / "escombros como spawns de props, que den piedra al picarse"): dos piezas nuevas, ambas reusando mecanismos ya existentes en vez de inventar uno nuevo, siguiendo la regla del proyecto de que un detalle nuevo es siempre un objeto de catálogo, nunca un tipo de terreno nuevo:
  - **Barro de orilla**: `calcularTerrenoTile` (`generar.js`) mira los 4 vecinos inmediatos de cada casilla de tierra (nunca la propia agua, ni camino/puente, ni un acantilado); si está pegada a un río/lago Y una capa de ruido de baja frecuencia (`orillabarro`, escala 14, umbral 0.58) lo dice, esa casilla se pinta de `barro`/`barro_b` (ya existían en el catálogo, solo se usaban dentro de pantano — ahora también aparecen como orilla de cualquier bioma) en vez del terreno normal — resultado: parches de barro salpicados por la orilla, no el perímetro entero del río.
  - **`yacimiento_arcilla`** (`rocas.json`, categoría nueva `arcilla`): sale por el decorador de densidad NORMAL, con un filtro nuevo `requiereBarro` (`decoracion.js`, mismo patrón que `requiereCercaAgua`) que solo deja salir el recurso en las casillas que el punto de arriba pintó de barro — así la arcilla queda confinada a "algunas zonas" de la orilla, nunca toda.
  - **`escombros_acantilado`** (`rocas.json`, categoría `piedra_comun`): mismo patrón, filtro nuevo `requiereAcantilado` reusando el flag `esAcantilado` que ya calculaba `calcularTerrenoTile` para las rocas de acantilado deterministas — esta vez expuesto como una opción más del catálogo normal (como pidió el streamer: "spawns de props, como los arbustos/árboles"), no colocación forzada.
  - **`items/catalogo/items.json`** gana la entrada `arcilla` (familia "piedra": peso 2.5, stackMax 15) para que el nuevo `categoriaRecurso` tenga su ítem de inventario — mismo patrón 1-a-1 que documenta ese catálogo (`server/test/inventario.test.ts`, contador subido de 55 a 56).
  - Verificado con un bake de prueba: 35 casillas de barro (en parches, no todo el río), 9 `yacimiento_arcilla` y 17 `escombros_acantilado` colocados, validación OK, 99/99 tests del server en verde.
- **Cuatro detalles más, mismo día, pedido del streamer** ("rocas erráticas, caminos más gastados y con más deco, densidad gradiente de vegetación, playas graduales — adelante con esto"):
  - **`roca_erratica`** (`rocas.json`): bloque grande suelto en llano SIN relación con el bioma (pradera/bosque/desierto/pantano), `densidadBase` muy baja (0.0008) a propósito — es un hito raro, no decoración habitual.
  - **Caminos más anchos cerca de la ciudad**: `marcarSegmentoComoCamino` ya no recibe un `radio` fijo — `radioCaminoEn(x,y)` da radio 1 (3 casillas de ancho) dentro de `RADIO_CAMINO_ANCHO=130` casillas del centro de la ciudad, radio 0 (sendero de 1 casilla) más allá. Junto con `mojon_camino` (`rocas.json`, `requiereJuntoCamino`+`requiereCercaCiudad`, dos flags nuevos de celda calculados baratos: `juntoCamino` reusa el `Set` global `tilesCaminoRoad` ya calculado, sin problema de borde de chunk) — mojones solo pegados a un camino de verdad y dentro del mismo radio ancho, así el camino "se nota más cuidado" cerca de casa y se queda pelado en las ramas lejanas.
  - **Gradiente de vegetación en el borde de bioma**: nuevo campo de celda `factorBordeBioma` — 4 muestras de `biomaGridSuave` a 4 casillas de distancia (N/S/E/O, ya calculado, sin coste de un distance-transform completo) dan qué fracción del entorno inmediato es el MISMO bioma; `decoracion.js` multiplica la densidad efectiva por ese factor SOLO para vegetación (rocas/fauna no se apagan igual cerca de una costura de bioma) — de 1 (densidad normal, rodeado del mismo bioma) a un suelo de 0.15 (nunca cero de golpe) justo en el borde.
  - **Playas graduales**: reusa el slot `playa_b` ya existente en el catálogo (36 terrenos, sin hueco libre — cambiar de sentido un slot existente en vez de pedir uno nuevo) — antes era puramente cosmético al azar (subvariante), ahora `calcularTerrenoTile` lo fuerza de forma DETERMINISTA en cualquier casilla de playa que toque agua de verdad (`esAguaEnTile`, unifica río/lago de cualquier banda + mar/agua baja por elevación banda≤1, mismas 4 consultas ya calculadas para el barro) — arena húmeda oscura (`colorDebug` bajado de `#d8c68e` a `#9c8358`) pegada al agua, arena seca normal el resto de la playa.
  - Verificado: el "1 sin ruta" que salió en 2 de 4 semillas de prueba SE REPRODUCE IGUAL con el código de ANTES de esta pasada (comprobado con `git stash` sobre los mismos archivos) — no lo causó nada de esto, es un caso raro preexistente de POI de cueva en pantano, no bloquea el bake (`validarMapa` solo avisa, no revienta). 91 `roca_erratica` + 23 `mojon_camino` en la semilla más generosa de las probadas.
...
- **Fauna acuática de agua dulce vs. mar** (pedido 2026-08-29, "peces de rio peces de mar salada de lagos"): hasta ahora TODA la fauna acuática (`requiereAgua: true`) estaba atada a biomas `mar_bajo`/`mar_profundo` — mecánicamente, ni un solo pez podía salir en un río o lago (la hidrología es un overlay sobre el bioma de TIERRA, `celda.bioma` de un río en pradera sigue siendo "pradera", nunca "mar_bajo"). Corregido con un nuevo dato de casilla, `celda.aguaDulce` (`generar.js`, calculado de `hidro.esRio`/`esLago`), que `decoracion.js` exige vía `requiereAguaDulce` en el catálogo — peces de mar y de agua dulce nunca se mezclan en la misma agua. **Bug real encontrado en esta misma pasada**: el campo se calculaba y se comprobaba, pero se quedaba sin pasar de `celda` a las `opciones` que de verdad lee `objetosEnCasilla` — cero peces de río salían pese a que la lógica estaba "completa"; corregido en `generarParaChunk`. 10 especies nuevas de río/lago (trucha, salmón, carpa, lucio, siluro, perca, trucha alpina, pez gato de oasis en desierto) + 6 de mar (arenque, bacalao, atún, pez espada, anguila marina costera, medusa, foca) sobre las 17 que ya había — tamaño/sociabilidad por el patrón de siempre (`escalaBase` + `densidadBase`/`capacidadMaximaPorChunk`: pequeño+banco como el arenque, grande+solitario como el siluro o el pez espada), sin campo nuevo de "grupal". El sistema NO distingue mecánicamente río de lago (ambos son la misma hidrología "agua dulce" de su bioma) — la separación río/lago en el catálogo es temática (nombre y `categoriaRecursoCarne`), no un filtro de sitio real; separarlos de verdad requeriría un dato de hidrología nuevo por casilla, no hecho en esta pasada.
