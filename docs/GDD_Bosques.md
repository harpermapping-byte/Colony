...
**ESTADO: v1 IMPLEMENTADA Y VERIFICADA (2026-08-30), alcance Hub (mapa principal), incluida la reconciliación visual del bake (§7).** Piezas: `baker/catalogo/vegetacion.json` (`crecimiento` en 28 especies maderables), `items/catalogo/items.json` (28 `semilla_<especie>`, `EntradaItem.crecimientoArbol`), `server/src/mundo/crecimientoBosques.ts` (reglas puras), `server/src/mundo/bosqueSector.ts` (resolución de sector), `server/src/mundo/bosquesVivos.ts` (`GestorBosques`, activación en vivo), `server/src/mundo/recolectables.ts` (`recolectablesQuitadosDeMapa`), `server/src/datos/bd.ts` (tablas `arboles_vivos`/`arboles_sector_resuelto`, 4 funciones), `server/src/rooms/schema/HubState.ts` (`ArbolVivoSchema`), `server/src/rooms/base/RoomExteriorBase.ts` (mensaje `sector:exclusiones`, hook `arbolesTaladosEnSector`), `server/src/rooms/HubRoom.ts` (wiring + protocolo `arbol:*` + broadcast `arbol:baketalado`), `client/src/render3d/sectorVisual.ts` (exclusión al construir + ocultado en vivo), `client/src/mapa/streamingSectores.ts` (`obtenerHandle`), `client/src/game.ts` (render de árboles nuevos + tecla H + reconciliación del bake). Probado: `crecimientoBosques.test.ts` (6), `bosqueSector.test.ts` (9), `arbolesVivos.test.ts` (4, persistencia BD), `bosquesVivos.test.ts` (10, gestor en vivo), `streaming.test.ts` (7, sin regresión), suite completa del server en verde, `tsc --noEmit` limpio en server y client.
...
~~**Sin tecla de atajo todavía**: a diferencia de talar (sin argumentos), plantar necesita elegir QUÉ semilla del inventario — se deja para cuando exista la interfaz de inventario en rejilla~~ **RESUELTO (auditoría de interacciones 2026-09-13)** — no hacía falta ninguna interfaz de inventario nueva: el menú de clic sobre el suelo (`mostrarMenuSueloClic`, `client/src/game.ts`, mismo componente que ya ofrecía "Labrar aquí"/"Cavar tierra" en terreno sin labrar) reúne las semillas de árbol del inventario del jugador por el campo de catálogo `crecimientoArbol.especieArbolId` (distinto de `.cultivo`, el que usan las semillas de bancal) y ofrece una opción "Plantar `<especie>` (N)" por cada una, mandando `arbol:plantar {x,y,instanciaId}` real. Antes de este fix la semilla se quedaba sin NINGÚN uso posible en el inventario — confirmado por grep que ningún fichero de cliente mandaba nunca `arbol:plantar` desde que existe la mecánica. `arbol:error`/`arbol:plantado` ganan además un toast real (antes solo consola). Verificado con `client/test/arbolPlantarClic.e2e.cjs` (servidor+Vite+Playwright reales): dar una `semilla_roble` real, clicar terreno abierto ofrece "Plantar Semilla de Roble (1)", clicarla confirma el toast "Has plantado un roble."
...
## 7. Reconciliación visual del bake — HECHO (2026-08-30): un árbol talado (o algo recolectado) desaparece de verdad

**Corrección real, resuelta el mismo día que se detectó**: los árboles del bake siguen siendo decoración estática (`InstancedMesh`/clones `.glb` por sector, cliente) y **nunca** se publican en `ArbolVivoSchema` — eso no cambió. Lo que se arregló fue el pipeline de streaming de sectores para que sepa, posición a posición, qué ya no existe:

- **Al materializar un sector** (`client/src/render3d/sectorVisual.ts::crearSectorVisual`, nuevo parámetro `excluidos: Set<string>`): antes de instanciar CUALQUIER prop del bake (árbol, pero también roca/fauna decorativa/vegetación — el mecanismo es genérico por posición, no específico de árboles), el cliente pide al servidor (`sector:exclusiones`, `RoomExteriorBase.ts`, request/response correlado por sector con timeout de 3s de seguridad) qué posiciones de ESE sector ya están vacías, y directamente no las dibuja. El servidor junta dos fuentes: recolectables ya cogidos (`recolectablesQuitadosDeMapa`, `server/src/mundo/recolectables.ts` — nuevo, mismo ciclo de vida sin persistencia que el pool activo) y árboles de origen bake talados (hook `arbolesTaladosEnSector`, sobreescrito solo por `HubRoom` vía consulta directa a `arboles_vivos`).
- **En vivo, con el jugador mirando** (`ocultarPosicion` del `HandleSector` que devuelve `crearSectorVisual`): si el sector YA está materializado y algo desaparece delante del jugador, se apaga esa instancia concreta sin reconstruir el sector entero — matriz a escala 0 en el camino `InstancedMesh` (no hay "quitar instancia" nativo en Three.js), `visible = false` en el camino de clones `.glb` reales. Disparado por dos broadcasts: `mundo:objetoQuitado` (recolectables — YA existía en el servidor desde `docs/GDD_Inventario.md` §7, pero nadie lo escuchaba en el cliente hasta ahora) y `arbol:baketalado` (nuevo, solo para árboles de origen bake — los "crecidos" no lo necesitan, su remove de `ArbolVivoSchema` ya lo hace todo solo).

**Alcance real de este arreglo**: cubre árboles talados y recolectables ya cogidos (vegetación con `desaparaceAlRecolectar`) — los dos casos donde HOY existe una acción de servidor que hace desaparecer algo del bake. Rocas y fauna decorativa NO tienen todavía ningún verbo de "quitar" (`rocas.json`/`animales.json` no tienen ninguna entrada `desaparaceAlRecolectar`, sin mecánica de minado) — el mecanismo cliente ya es genérico por posición y los cubriría sin cambios el día que exista esa acción server-side, pero hoy no hay nada que reconciliar ahí porque nada los quita.
...
## 10. Deforestación local permanente — CONFIRMADO A PROPÓSITO, no un gap (2026-09-08)

Pedido streamer, al revisar el estado de talar/respawn: si se tala TODO árbol adulto de una zona, no queda ningún adulto vivo que pueda propagar (§2 exige un árbol adulto elegible como origen de la tirada) — esa zona se queda deforestada para siempre, sin ningún mecanismo que la resiembre sola. Confirmado explícitamente que esto se queda TAL CUAL — "la gracia" del sistema es justo esa: la responsabilidad recae en los jugadores (talar con cabeza, plantar semillas de vuelta con `arbol:talar`+`arbol:plantar`, §4/§5) en vez de que el juego amortigüe el abuso solo. No se ha tocado ni se va a tocar código para "arreglar" esto — es el comportamiento pretendido, documentado aquí para que ninguna sesión futura lo confunda con un bug y lo "arregle" sin preguntar. Mismo criterio aplicado en paralelo a la extinción local de fauna reproductora, ver `docs/GDD_Agentes_Moviles.md` fase 9.
