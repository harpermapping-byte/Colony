...
## Entrar a un asentamiento te deja PEGADO a su portal de salida (RESUELTO, 2026-09-10)

Bug real del playtest multijugador (`client/test/playtestMultijugador.e2e.mjs`, paso 7): cruzar la puerta de `capital_regional` desde el exterior funcionaba, pero al entrar el jugador aparecía en el spawn del bake (`indice.ciudad`, el centro de la plaza — 50,130) y el único portal de salida cercano está en (46,132), a 4.5 casillas: fuera de `RADIO_INTERACCION` (2.2), así que F respondía "no hay puerta cerca" y había que adivinar dónde estaba la salida. Causa: `HubRoom` manda `portal:ir {tipo:"region", mapaId}` SIN coordenadas, y `RegionRoom.onJoin` sin `entradaX/Y` caía al spawn genérico.

Arreglo (`RegionRoom.puntoDeEntradaPorDefecto`): sin `entradaX/Y`, el punto por defecto es el portal `tipo:"exterior"` sin destino más cercano al spawn del bake (si su casilla es sólida — el arco de la muralla — la pisable más cercana con salida, `casillaPisableMasCercana(..., conSalida)`); sin portales exteriores (arena, `capital_jarl` suelta) sigue siendo el spawn de siempre. `entradaX/Y` explícitos (escaleras, otra planta) y la posición guardada de un F5 (`resolverSpawnGuardado`) mandan como antes. E2E: `server/test/entradaRegionPuerta.e2e.mjs` (entra sin coordenadas → a ≤2.2 casillas de un portal de salida → `portal:usar` devuelve `portal:ir {tipo:"volver"}`; con `entradaX/Y` explícitos se respeta ese punto).

## Puerta física CLICABLE — etiqueta "Entrar <Nombre>" (RESUELTO, 2026-09-12)

Pedido streamer: "puerta física clicable -> entrar a la instancia". Hasta ahora la ÚNICA forma de cruzar cualquier puerta/portal (interior de un edificio, exterior de un asentamiento anidado) era pulsar F a ciegas cerca de ella — sin ninguna pista visual de que hubiera algo interactivo ahí, ni nombre del destino. Cierra ese hueco con una etiqueta flotante clicable, sin tocar el protocolo de red existente (F sigue funcionando exactamente igual).

### 1. Bake: 3 campos nuevos, aditivos, en el portal `tipo:"exterior"`

`baker/src/instanciasPOI.js::colocarSiluetaYPuertaDeAsentamiento` (las dos ramas: con puertas reales del polígono de muralla, y el fallback sin ninguna puerta real detectada) añade al objeto que entra en `portales.push(...)`:

- **`puertaX`/`puertaY`** — coordenadas CONTINUAS del centro real del arco/puerta física (`generar_puerta_asentamiento.js`), YA calculadas ahí mismo para colocar la estructura — DISTINTAS de `x,y` (la casilla de PORTAL, empujada 2-4.24 casillas fuera del polígono de muralla real, ver §13quater de `docs/GDD_Bakeador_POIs.md` — el punto que el servidor de verdad comprueba con `RADIO_INTERACCION`).
- **`nombreDestino`** — nombre legible derivado del `id` de catálogo del POI (`nombreLegibleDesdeId`, nuevo helper local: `_.replace(/_/g,' ')` + capitaliza cada palabra con `\b\p{L}/gu`, MISMO criterio ya usado en `client/src/cocina/panelCocina.ts::nombreVasija` — nunca una tabla curada a mano, CLAUDE.md filosofía #7). `"aldea_agricola"` → `"Aldea Agricola"`, `"capital_regional"` → `"Capital Regional"`.

Las ramas "edificio" y "mazmorra" (portales `tipo:"interior"`, donde `x,y` YA es la puerta real — no hay arco separado del punto de portal) ganan solo `nombreDestino`, sin `puertaX/Y` (no hace falta duplicar la posición). Los 3 campos son **opcionales** en el tipo (`PortalMapa`/`Portal`, cliente y servidor) — un mapa horneado ANTES de esta fecha (incluido `assets/mapas/principal/` hasta su próximo rebake) simplemente no los trae y no ofrece ninguna etiqueta; la tecla F sigue funcionando idéntica.

### 2. Cliente: etiqueta CSS2D real, mostrada/ocultada por distancia

`client/src/game.ts` (junto a donde ya se carga `indiceMapa`) filtra `indice.portales` a los `tipo:"exterior"` con `destino`+`puertaX/Y`+`nombreDestino` (hoy solo los produce la rama de asentamiento — civil u hostil, da igual, ambas usan la misma función de bake) y crea, por cada una, una etiqueta con `WorldScene.añadirEtiquetaInteractiva(id, puertaX, puertaY, alturaY, "Entrar <Nombre>", onClick)` (nuevo método, mismo mecanismo `CSS2DObject` que ya usan los nombres de jugador/NPC). Un `setInterval` a 400ms (mismo ritmo que `hintAsiento`) mide la distancia del jugador local al **portal real** (`p.x+0.5, p.y+0.5` — el mismo punto y offset que el servidor comprueba en `RegionRoom`/`HubRoom.onMessage("portal:usar")`, NUNCA la puerta visual, que puede estar más lejos) contra `RADIO_ENTRADA_PORTAL_CLIENTE = 2.2` (mismo valor que `RADIO_INTERACCION` del servidor) y la muestra/oculta — así la etiqueta solo se ofrece cuando el clic va a funcionar de verdad, nunca una aproximación que pueda desincronizarse. Al clicarla (`stopPropagation` para no colar el clic al raycast del mundo 3D detrás) manda `room.send("portal:usar")` — EL MISMO mensaje que ya manda la tecla F, cero cambio de servidor.

**`portal:error`** (antes solo `console.log`, invisible jugando) ahora también sale como toast (`registroCombate.mostrar`, mismo patrón que `combate:error`/`cofre:error`/`asiento:error`).

### Dos bugs reales de `CSS2DRenderer` encontrados escribiendo el e2e, ninguno visible con `tsc`/tests unitarios

1. **`render()` de `CSS2DRenderer` reescribe `element.style.display` en CADA frame** según su propio criterio de profundidad/capa, pisando cualquier `div.style.display` puesto a mano desde fuera en el frame siguiente — `mostrarEtiquetaInteractiva` alterna `div.style.visibility` en su lugar (que `CSS2DRenderer` jamás toca).
2. **`objeto.visible = false` para "crearla oculta" (primera versión) estaba mal**: `renderObject()` comprueba `object.visible===false` ANTES que nada y, si es `false`, llama a `hideObject()` y hace `return` sin llegar NUNCA a la línea que inserta el `<div>` en el DOM — el elemento no existía en el documento en absoluto hasta la primera vez que se marcara visible. `objeto.visible` se deja SIEMPRE en su valor por defecto (`true`); el propio filtro de "visible" de `renderObject()` solo mira la profundidad proyectada (`_vector.z`), nunca los límites X/Y de pantalla, así que un objeto lejos del jugador se sigue insertando en el DOM igual (con un `transform` que lo coloca fuera de vista) — `visibility:hidden/visible` decide de verdad si se ve y si es clicable (un elemento `visibility:hidden` no recibe eventos de puntero).

### Verificación

`client/test/puertaFisicaClicable.e2e.cjs` (servidor+Vite+Playwright reales): hornea EN PROCESO un mapa de prueba pequeño y real (mismo config que `baker/config/ejemplo-rapido.json`, vía `generarMapa` sin pasar por el CLI, a una carpeta temporal bajo `assets/mapas/`, borrada en el `finally` — mismo patrón que `mazmorraLimpiada.e2e.cjs`) y relee su `indice.json` con el MISMO filtro que aplica `game.ts` para saber cuál es "portal_0" sin hardcodear nada. 10/10 comprobaciones en verde: la etiqueta existe en el DOM con el texto exacto "Entrar Poblado Orco Poi" (creada oculta), oculta lejos de la puerta, `portal:usar` lejos da un toast visible con "puerta" en pantalla, se hace visible junto a la puerta real, clic REAL con `page.mouse.click` sobre sus coordenadas de pantalla (nunca se invoca el handler a mano), navegación real a `sala=region` con el `mapaId` resuelto correctamente (`resolverMapaIdDestino`, `<mapaIdPropio>/pois/<slug>`), la región de destino carga de verdad tras la recarga, sin etiqueta residual de la sala anterior, cero errores de consola/página. Captura real en `client/test/capturas/puerta_fisica_etiqueta_visible.png`. `tsc --noEmit` limpio en cliente y servidor, servidor 1356/1356, cliente 111/111 (sin test unitario nuevo — el hueco real estaba en la integración con `CSS2DRenderer`, que solo un navegador real ejercita; queda cubierto por el e2e).

**Pendiente real, honesto**: `assets/mapas/principal/` (el mapa de producción ya promocionado) NO tiene estos 3 campos hasta su próximo rebake — el streamer decide cuándo rehornear, esta pasada no lo hizo (norma del proyecto: "los bakes grandes los corre el usuario"). Los portales `tipo:"interior"` (puertas de edificio/mazmorra sueltos) ganaron `nombreDestino` en el bake pero el cliente todavía NO les pinta ninguna etiqueta (el filtro de `game.ts` exige `destino`, que solo llevan los "exterior" de asentamiento) — extender la etiqueta a puertas de edificio normales es una ampliación futura, no pedida en esta pasada.
