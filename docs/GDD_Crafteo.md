...
## 10. Mesas de oficio siempre presentes + panel de crafteo genérico + coste de materiales para construirlas (2026-09-10/11, pedido streamer: "cada mesa que se incluya como mueble en los edificios de ese oficio, obligatorio siempre 1 o 2 mesas de cada oficio en cada edificio de oficio... y luego que se puedan construir, las de nivel 1 de crafteo fácil con material fácil, las siguientes complicándose con materiales de otras mesas o más elaborados")

Tres piezas encadenadas: primero se confirmó (generando edificios reales, no solo leyendo catálogo) que dos edificios de oficio podían salir SIN ninguna mesa funcional de su propio oficio; después se construyó el panel de cliente que "crafteo no tiene panel de cliente hoy" (§5/§7ter) llevaba pendiente desde el diseño original; y por último se le puso coste de materiales a CONSTRUIR esas mesas con la tecla B, cerrando el `receta` que `docs/GDD_Construccion.md` ya reservaba desde el principio ("se rellenará al definir la economía... sin receta = gratis por ahora").
...
### 10.2 Panel de crafteo genérico de cliente

Cierra "crafteo no tiene panel de cliente hoy" (§5) para CUALQUIER mesa de oficio (excepto `caldero`, que abre el panel de alquimia — ver `docs/GDD_Pociones.md` §10). Mensaje nuevo `crafteo:recetasDisponibles {construccionId}` (`RoomExteriorBase.ts::manejarCrafteoRecetasDisponibles`) devuelve, para la mesa concreta donde el jugador hace clic, TODAS las recetas cuyo `mesas` la incluye, con nivel de oficio actual, insumos, resultado, y flags `bloqueadaPorNivel`/`edificioFaltante`/`planoFaltante` ya resueltos server-side (el cliente nunca decide si algo se puede craftear, solo lo pinta). `client/src/construccion/panelCrafteo.ts` (nuevo, `crearMarcoPanel`, icono 🛠): agrupa recetas por nivel mínimo con candado visual en las bloqueadas, botón "Craftear"/"Recolectar" según el estado, barra de progreso mientras dura el crafteo. Se abre con clic sobre cualquier mesa con `nivelOficioMinimo` (opción de menú "Craftear en <mesa>"). **Mismo patrón de bug de re-render encontrado y cerrado que ya documentaba `docs/GDD_UI_Paneles.md`**: el intervalo de refresco de la cuenta atrás seguía re-renderizando después de completarse el crafteo, causando clics fantasma sobre "Recolectar" detectados por Playwright (`element was detached from the DOM, retrying`) — arreglado deteniendo el intervalo en cuanto se cruza el umbral, un único render final.

### 10.3 Coste de materiales para construir una mesa de oficio (tecla B)

`EntradaConstruible`/`EntradaElemento`/`EntradaExterior` (`server/src/construccion/catalogo.ts`) ganan un campo opcional `receta: {itemId, cantidad}[]` — el mismo campo que `docs/GDD_Construccion.md` ya reservaba sin implementar. Aplicado a los 61 elementos con `nivelOficioMinimo` en `elementos.json` (generado con una tabla `MATERIAL_POR_TEMA` + una fórmula `recetaParaNivel(mat, nivel)` que escala: nivel 1 = material bruto barato y poca cantidad, niveles altos = material PROCESADO del propio oficio — nunca de otro oficio, decisión confirmada con el streamer — en cantidad creciente) y a las 6 vasijas de cocina en `exteriores.json` (madera+piedra básicos, son nivel 1). El handler `"construir"` (`RoomExteriorBase.ts`) valida que el inventario tenga TODOS los insumos antes de construir, los descuenta de verdad, y los devuelve íntegros si la construcción se revierte por cualquier otro motivo de validación posterior. El cliente (`constructor.ts`) muestra un tooltip con "Requiere nivel N de <oficio>. Materiales: X, Y" en cualquier mesa con coste.

### Verificado (§10, conjunto)

Servidor 1321/1321, `tsc --noEmit` limpio en cliente y servidor, cliente 73/73. E2E nuevos: `client/test/panelCrafteo.e2e.cjs` (12/12 — lista de recetas agrupada por nivel, craftear/recolectar real, inventario Schema replicado confirma el descuento/entrega reales), `server/test/costeConstruirMesaOficio.e2e.mjs` (5/5 — rechazo sin materiales, éxito+descuento real con materiales, vía colyseus.js directo). `server/test/megaEstresTodasLasMecanicas.e2e.mjs` actualizado para seguir sembrando materiales suficientes al dueño de parcela (no re-ejecutado completo, coste alto de esa suite). Detalle de la mecánica de frasco de poción y el panel de alquimia, en la misma tanda de peticiones, en `docs/GDD_Pociones.md` §10.
