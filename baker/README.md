...
- **Nota 2026-08-29 (pedido del streamer)**: tres frentes, ver `docs/GDD_Bakeador_Exteriores.md` secciones 2/9 para el detalle completo — (1) subbiomas cosméticos de textura (2-3 looks por bioma, `terreno.js`/`SUBVARIANTES`), tope de 36 tipos de terreno alcanzado y documentado (`terrenos.json`, `_nota_limite_36` — la codificación de casilla es 1 carácter base-36); (2) rocas de acantilado placeholder por pendiente real (radio 5, no por salto inmediato de banda — medido que ese salto casi nunca ocurre con el ruido de elevación actual, deliberadamente suave para un mundo caminable); (3) peces de río/lago separados de los de mar (`celda.aguaDulce`), 16 especies nuevas. Bug real encontrado y corregido en el mismo pase: el dato `aguaDulce` se calculaba pero no llegaba de `celda` a las `opciones` que de verdad lee el filtro de elegibilidad — cero peces de agua dulce salían pese a que el resto de la tubería estaba completo.
