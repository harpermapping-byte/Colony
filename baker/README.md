...
- **Más variantes visuales en especies comunes**: para que una especie que se repite mucho (mismo recurso, p.ej. madera de pino) no se vea siempre igual, se subió el número de `variantes` de las especies más comunes de bosque y pradera (el pino pasa de 4 a 8 variantes visuales, por ejemplo) — mismo recurso al recolectar, aspecto distinto al verlo.
